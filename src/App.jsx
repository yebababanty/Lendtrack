...
const VARIABLE_HOLIDAYS = new Set(["2024-03-29","2024-04-01","2025-04-18","2025-04-21","2025-03-30","2025-03-31","2026-04-03","2026-04-06","2026-03-20","2026-03-21","2026-05-27","2026-05-28"]);
...
const fm = (ym) => { if(!ym) return "—"; const[y,m]=ym.split("-"); return new Date(y,parseInt(m)-1,1).toLocaleDateString("en-NG",{month:"long",year:"numeric"}); };
...
function getRepayDays(start, count, excl) {
  ...
  while(days.length<count){ const ds=cur.toISOString().split("T")[0],day=cur.getDay(); if(excl){if(day!==0&&day!==6&&!isHoliday(ds))days.push(ds);}else days.push(ds); cur.setDate(cur.getDate()+1); }
  return days;
}
...
function calcLoanSchedule(principal, rate, days, start, excl) {
  ...
  const repDays=getRepayDays(start,days,excl); const schedule=[]; let bal=total;
  for(let i=0;i<days;i++){ bal=Math.max(0,bal-daily); schedule.push({day:i+1,dueDate:repDays[i],payment:daily,balance:bal,paid:false,paidDate:null,paidAmount:0,overpayment:0,shortfall:0,paymentLog:[]}); }
  return {dailyPayment:daily,totalRepayable:total,totalInterest:interest,schedule};
}

// ─── SMART PAYMENT ENGINE (with payment log) ─────────────────────────────────
function applySmartPayment(schedule, startIdx, amountPaid, paidDate, paidBy) {
  ...
  while(remaining>0&&i<updated.length){
    ...
    if(i===startIdx){
      ...
      // Add to payment log
      updated[i].paymentLog.push({amount:amountPaid,date:paidDate,by:paidBy||"",at:new Date().toISOString()});
      updated[i]={...updated[i],paid:total>=s.payment,paidAmount:covers,paidDate,overpayment:Math.max(0,excess),shortfall:Math.max(0,s.payment-total)};
      ...
      updated[i].paymentLog.push({amount:here,date:paidDate,by:paidBy||"",at:new Date().toISOString(),cascaded:true});
      updated[i]={...updated[i],paid:newPaid>=s.payment,paidAmount:newPaid,paidDate:newPaid>=s.payment?paidDate:s.paidDate,overpayment:0,shortfall:Math.max(0,s.payment-newPaid)};
      remaining-=here;
    }
    i++;
  }
  ...
  let bal=totalDue;
  for(let j=0;j<updated.length;j++){ bal=Math.max(0,bal-(updated[j].paidAmount||0)); updated[j].balance=bal; }
  return updated;
}
...
function computeFinancials(clients) {
  ...
  clients.forEach(c=>{(c.loans||[]).forEach(l=>{
    const p=l.principal||0,int=l.totalInterest||0,td=l.totalRepayable||0;
    const iR=td>0?int/td:0,pR=td>0?p/td:0;
    totalCapital+=p; totalExpectedInterest+=int;
  ...
  return {totalCapital,totalExpectedInterest,interestEarned,principalCollected,outstandingPrincipal:Math.max(0,totalCapital-principalCollected),trueProfit:interestEarned,realROI:totalCapital>0?(interestEarned/totalCapital)*100:0,collectionRate:(totalExpectedInterest+totalCapital)>0?(totalCollected/(totalExpectedInterest+totalCapital))*100:0,totalCollected};
}
...
function computeClientFinancials(client) {
  ...
  (client.loans||[]).forEach(l=>{
    const p=l.principal||0,int=l.totalInterest||0,td=l.totalRepayable||0;
    const iR=td>0?int/td:0,pR=td>0?p/td:0;
    capital+=p; expectedInterest+=int;
    (l.schedule||[]).forEach(s=>{if(s.paidAmount>0){totalCollected+=s.paidAmount;interestEarned+=s.paidAmount*iR;principalCollected+=s.paidAmount*pR;}});
  });
...
function computeLoanFinancials(loan) {
  const p=loan.principal||0,int=loan.totalInterest||0,td=loan.totalRepayable||0;
  const iR=td>0?int/td:0; let collected=0,intEarned=0,prinCollected=0;
  const pR=td>0?p/td:0;
  (loan.schedule||[]).forEach(s=>{if(s.paidAmount>0){collected+=s.paidAmount;intEarned+=s.paidAmount*iR;prinCollected+=s.paidAmount*pR;}});
  const paidCount=(loan.schedule||[]).filter(s=>s.paid).length, totalCount=loan.days||1, today=new Date();
  return {collected,interestEarned:intEarned,principalCollected:prinCollected,outstandingPrincipal:Math.max(0,p-prinCollected),trueProfit:intEarned,realROI:p>0?(intEarned/p)*100:0,collectionRate:td>0?(collected/td)*100:0,completionRate:(paidCount/totalCount)*100,paidCount,totalCount,overdueCount:(loan.schedule||[]).filter(s=>!s.paid&&new Date(s.dueDate)<today).length,onTimeCount:(loan.schedule||[]).filter(s=>s.paid&&s.paidDate<=s.dueDate).length,lateCount:(loan.schedule||[]).filter(s=>s.paid&&s.paidDate>s.dueDate).length,partialCount:(loan.schedule||[]).filter(s=>!s.paid&&s.paidAmount>0&&s.paidAmount<s.payment).length};
}

function computeMonthlyStats(clientsList, monthKey) {
  let totalDisbursed=0,totalExpected=0,totalCollected=0,overdueCount=0; const today=new Date();
  clientsList.forEach(c=>{(c.loans||[]).forEach(l=>{
    if((l.issuedAt||l.startDate||"").slice(0,7)===monthKey) totalDisbursed+=l.principal||0;
    (l.schedule||[]).forEach(s=>{if((s.dueDate||"").slice(0,7)===monthKey){totalExpected+=s.payment||0;if(s.paidAmount>0)totalCollected+=s.paidAmount;else if(new Date(s.dueDate)<today)overdueCount++;}});
  });});
...
// ─── MONTHLY DISBURSEMENT REPORT (ADMIN) ──────────────────────────────────────
function computeMonthlyReport(clients) {
  ...
  clients.forEach(c=>{(c.loans||[]).forEach(l=>{
    ...
    map[key].disbursed+=l.principal||0; map[key].expected+=l.totalRepayable||0; map[key].interest+=l.totalInterest||0; map[key].loanCount++;
    map[key].clientIds.add(c.id);
    (l.schedule||[]).forEach(s=>{if(s.paidAmount>0)map[key].collected+=s.paidAmount;});
  });});
  return Object.entries(map).map(([month,d])=>({month,...d,clientCount:d.clientIds.size,outstanding:Math.max(0,d.expected-d.collected)})).sort((a,b)=>b.month.localeCompare(a.month));
}

// ─── STAFF MONTHLY REPORT (ADMIN) ────────────────────────────────────────────
        ...
        (l.schedule||[]).forEach(s => {
          if ((s.dueDate||"").slice(0,7) === monthKey) {
            ...
            if (s.paidAmount > 0) {
              ...
              // Count individual payments from paymentLog
              (s.paymentLog||[]).forEach(pl => {
                if ((pl.date||"").slice(0,7) === monthKey) paymentsReceived++;
              });
              // Fallback if no paymentLog
              if (!s.paymentLog || s.paymentLog.length === 0) paymentsReceived++;
            }
...
function getClientTransactions(client) {
  ...
  (client.loans||[]).forEach((l,lIdx)=>{
    // Individual payment entries from paymentLog
    (l.schedule||[]).forEach(s=>{
      if(s.paymentLog&&s.paymentLog.length>0){
        s.paymentLog.forEach((pl,plIdx)=>{
          txs.push({
            ...
            detail:`${pl.by?`by ${pl.by} · `:""}${fd(pl.date)}${pl.cascaded?" · Auto-cascaded from overpayment":""}`,
            color:pl.cascaded?"#a78bfa":"#4ade80",icon:pl.cascaded?"⚡":"✓",
          });
        });
      } else if(s.paidAmount>0){
        // Fallback for old data without paymentLog
        txs.push({
    ...
    // Loan issuance
    txs.push({
      id:`issued-${l.id}`,type:"loan_issued",date:(l.issuedAt||l.startDate||"").split("T")[0],amount:l.principal,
...
// ─── UI COMPONENTS ────────────────────────────────────────────────────────────
function Modal({title,onClose,children,wide}){return(<div style={{position:"fixed",inset:0,background:"rgba(0,8,20,0.92)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:16,backdropFilter:"blur(8px)"}}><div style={{background:"#0d1b2a",border:"1px solid rgba(100,200,255,0.12)",borderRadius:20,width:"100%",maxWidth:wide?580:480,maxHeight:"92vh",overflowY:"auto"}}><div style={{padding:"20px 22px 14px",borderBottom:"1px solid rgba(255,255,255,0.06)",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:0,background:"#0d1b2a",zIndex:1,borderRadius:"20px 20px 0 0"}}><h2 style={{margin:0,fontSize:17,fontWeight:700,color:"#e8f4fd"}}>{title}</h2><button onClick={onClose} style={{background:"rgba(255,255,255,0.06)",border:"none",color:"#8ab4c8",width:30,height:30,borderRadius:8,cursor:"pointer",fontSize:17}}>×</button></div><div style={{padding:"20px 22px"}}>{children}</div></div></div>);}
function Field({label,children}){return<div style={{marginBottom:14}}><label style={{display:"block",fontSize:11,color:"#5a7a90",marginBottom:5,letterSpacing:0.8,textTransform:"uppercase"}}>{label}</label>{children}</div>;}
...
function SC({label,value,accent,sub}){return(<div style={{background:"rgba(255,255,255,0.03)",border:"1px solid rgba(255,255,255,0.06)",borderRadius:14,padding:"16px 18px",borderTop:`2px solid ${accent}`}}><div style={{fontSize:10,color:"#4a6880",letterSpacing:1.2,textTransform:"uppercase",marginBottom:7}}>{label}</div><div style={{fontSize:19,fontWeight:800,color:"#e8f4fd",fontFamily:"'Courier New',monospace"}}>{value}</div>{sub&&<div style={{fontSize:10,color:"#3a5060",marginTop:3}}>{sub}</div>}</div>);}
function Badge({color,bg,children}){return<span style={{fontSize:10,padding:"2px 9px",borderRadius:20,background:bg,color,fontWeight:700}}>{children}</span>;}
function RB({role}){return role==="admin"?<Badge color="#f59e0b" bg="rgba(245,158,11,0.15)">👑 ADMIN</Badge>:<Badge color="#60a5fa" bg="rgba(96,165,250,0.15)">🏦 OFFICER</Badge>;}
function Toast({msg,type}){if(!msg)return null;return<div style={{position:"fixed",top:70,left:"50%",transform:"translateX(-50%)",background:type==="error"?"rgba(248,113,113,0.15)":"rgba(34,197,94,0.15)",border:`1px solid ${type==="error"?"rgba(248,113,113,0.3)":"rgba(34,197,94,0.3)"}`,color:type==="error"?"#f87171":"#4ade80",padding:"10px 20px",borderRadius:12,zIndex:9999,fontWeight:700,fontSize:13,backdropFilter:"blur(8px)"}}>{msg}</div>;}
function FMR({label,value,valueColor="#e8f4fd",icon,sub}){return(<div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",background:"rgba(255,255,255,0.02)",borderRadius:10,border:"1px solid rgba(255,255,255,0.04)"}}><div style={{display:"flex",alignItems:"center",gap:8}}>{icon&&<span style={{color:valueColor,opacity:0.8}}>{icon}</span>}<div><div style={{fontSize:11,color:"#5a7a90"}}>{label}</div>{sub&&<div style={{fontSize:9,color:"#3a4a58",marginTop:1}}>{sub}</div>}</div></div><div style={{fontSize:14,fontWeight:800,color:valueColor,fontFamily:"'Courier New',monospace"}}>{value}</div></div>);}
function ROIGauge({roi}){const c=Math.min(Math.max(roi,0),100);const color=roi>=20?"#22c55e":roi>=10?"#f59e0b":"#ef4444";return(<div style={{marginTop:4}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:5}}><span style={{fontSize:10,color:"#5a7a90"}}>ROI</span><span style={{fontSize:12,fontWeight:800,color,fontFamily:"'Courier New',monospace"}}>{roi.toFixed(2)}%</span></div><div style={{height:6,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:`${c}%`,background:`linear-gradient(90deg,${color},${color}cc)`,borderRadius:4}}/></div></div>);}
function FP({fin,isClient,expandFinancials,setExpandFinancials}){const rc=fin.realROI>=20?"#22c55e":fin.realROI>=10?"#f59e0b":"#ef4444";return(<div style={{background:"linear-gradient(135deg,rgba(16,30,50,0.9),rgba(10,20,40,0.95))",border:"1px solid rgba(100,180,255,0.1)",borderRadius:16,overflow:"hidden",marginBottom:20}}><div style={{background:"linear-gradient(135deg,rgba(59,130,246,0.12),rgba(34,197,94,0.08))",padding:"14px 16px",borderBottom:"1px solid rgba(100,180,255,0.08)",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{display:"flex",alignItems:"center",gap:8}}><Icon name="trend" size={16}/><div style={{fontSize:13,fontWeight:800,color:"#e8f4fd"}}>{isClient?"Client Intelligence":"Portfolio Intelligence"}</div></div>{!isClient&&setExpandFinancials&&<button onClick={()=>setExpandFinancials(!expandFinancials)} style={{background:"rgba(100,180,255,0.08)",border:"none",borderRadius:8,color:"#60a5fa",padding:"5px 12px",fontSize:11,cursor:"pointer",fontWeight:600}}>{expandFinancials?"Hide":"Show"}</button>}</div>{(isClient||expandFinancials)&&(<div style={{padding:"14px 16px",display:"flex",flexDirection:"column",gap:8}}><FMR label="Capital" value={fc(fin.totalCapital??fin.capital)} valueColor="#60a5fa" icon={<Icon name="shield" size={13}/>}/><FMR label="Outstanding" value={fc(fin.outstandingPrincipal)} valueColor={fin.outstandingPrincipal>0?"#f87171":"#4ade80"} icon={<Icon name="shield" size={13}/>}/><FMR label="Interest Earned" value={fc(fin.interestEarned)} valueColor="#a78bfa" icon={<Icon name="trend" size={13}/>}/><div style={{padding:"12px 14px",background:"rgba(34,197,94,0.06)",borderRadius:10,border:"1px solid rgba(34,197,94,0.15)",display:"flex",justifyContent:"space-between",alignItems:"center"}}><div style={{fontSize:11,color:"#4ade80",fontWeight:700}}>TRUE PROFIT</div><div style={{fontSize:18,fontWeight:900,color:"#22c55e",fontFamily:"'Courier New',monospace"}}>{fc(fin.trueProfit)}</div></div><div style={{padding:"12px 14px",background:`${rc}14`,borderRadius:10,border:`1px solid ${rc}33`}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><div style={{fontSize:11,color:rc,fontWeight:700}}>ROI</div><div style={{background:`${rc}20`,borderRadius:8,padding:"4px 10px"}}><span style={{fontSize:16,fontWeight:900,color:rc,fontFamily:"'Courier New',monospace"}}>{fin.realROI.toFixed(2)}%</span></div></div><ROIGauge roi={fin.realROI}/></div></div>)}</div>);}

// ─── INSTALLMENT ROW (with payment log display) ──────────────────────────────
    ...
    <div style={{background:bg,border:`1px solid ${border}`,borderRadius:10,padding:"10px 12px",marginBottom:6}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
        <div style={{display:"flex",gap:9,alignItems:"flex-start"}}>
          ...
          <div>
            <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:3,flexWrap:"wrap"}}>
              ...
              {hasLog&&<button onClick={()=>setShowLog(!showLog)} style={{background:"rgba(245,158,11,0.1)",border:"1px solid rgba(245,158,11,0.2)",color:"#f59e0b",borderRadius:5,padding:"1px 6px",cursor:"pointer",fontSize:9,fontWeight:700}}>{s.paymentLog.length} payment{s.paymentLog.length>1?"s":""} {showLog?"▲":"▼"}</button>}
            </div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
          <div style={{textAlign:"right"}}><div style={{fontSize:8,color:"#3a5a70"}}>BAL</div><div style={{fontSize:10,color:"#8ab4c8",fontFamily:"'Courier New',monospace"}}>{fc(s.balance)}</div></div>
          {!s.paid&&isActive&&<button onClick={e=>{e.stopPropagation();onPay(loan,i);}} style={{background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",padding:"5px 10px",borderRadius:6,cursor:"pointer",fontWeight:700,fontSize:10}}>Pay</button>}
...
function LCC({loan,num,total,isAdmin,onPay,onOverride,today}){
  const[exp,setExp]=useState(num===total);const[schOpen,setSchOpen]=useState(num===total);
  const fin=computeLoanFinancials(loan);const isAct=loan.status==="active";
      ...
      <div onClick={()=>setExp(!exp)} style={{padding:"14px 16px",cursor:"pointer"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
            <div style={{width:38,height:38,borderRadius:10,flexShrink:0,background:isAct?"linear-gradient(135deg,#16a34a,#22c55e)":"linear-gradient(135deg,#1d4ed8,#3b82f6)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:13,fontWeight:800,color:"#fff"}}>{loan.status==="completed"?<Icon name="check" size={16}/>:`L${num}`}</div>
            <div><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}><span style={{fontSize:13,fontWeight:700,color:"#e8f4fd"}}>Cycle {num}</span><Badge color={isAct?"#4ade80":"#60a5fa"} bg={isAct?"rgba(34,197,94,0.12)":"rgba(96,165,250,0.12)"}>{isAct?"ACTIVE":"DONE"}</Badge></div><div style={{fontSize:10,color:"#3a5a70"}}>{fd(loan.startDate)} · {loan.days}d · {loan.interestRate}%</div></div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:8}}><div style={{textAlign:"right"}}><div style={{fontSize:13,fontWeight:800,color:"#60a5fa",fontFamily:"'Courier New',monospace"}}>{fc(loan.principal)}</div></div><div style={{color:"#3a5a70"}}>{exp?<Icon name="chevronUp" size={14}/>:<Icon name="chevronDown" size={14}/>}</div></div>
        </div>
        <div style={{marginTop:10}}><div style={{display:"flex",justifyContent:"space-between",fontSize:9,color:"#3a5a70",marginBottom:4}}><span>{fin.paidCount}/{fin.totalCount}</span><span style={{color:pc}}>{fin.completionRate.toFixed(0)}%</span></div><div style={{height:4,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:`${fin.completionRate}%`,background:`linear-gradient(90deg,${pc},${pc}99)`,borderRadius:4}}/></div></div>
      </div>
      {exp&&(<div style={{padding:"14px 16px"}}>
        <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:14}}>
          {[["Principal",fc(loan.principal),"#93c5fd"],["Daily",fc(loan.dailyPayment),"#60a5fa"],["Total",fc(loan.totalRepayable),"#e8f4fd"],["Collected",fc(fin.collected),"#4ade80"],["Outstanding",fc(Math.max(0,loan.totalRepayable-fin.collected)),fin.outstandingPrincipal>0?"#f87171":"#4ade80"],["Interest",`${loan.interestRate}%`,"#c4b5fd"]].map(([l,v,c])=>(<div key={l} style={{background:"rgba(255,255,255,0.02)",borderRadius:9,padding:"9px 10px",border:"1px solid rgba(255,255,255,0.04)"}}><div style={{fontSize:8,color:"#3a5a70",marginBottom:3}}>{l}</div><div style={{fontSize:11,fontWeight:700,color:c,fontFamily:"'Courier New',monospace"}}>{v}</div></div>))}
        </div>
        {isAdmin&&<div style={{background:"rgba(0,0,0,0.2)",borderRadius:12,padding:"12px 14px",marginBottom:14}}><div style={{fontSize:11,color:"#5a7a90",fontWeight:700,marginBottom:10}}>FINANCIAL INTELLIGENCE</div>{[["Interest Earned",fc(fin.interestEarned),"#a78bfa"],["Profit",fc(fin.trueProfit),"#22c55e"],["ROI",`${fin.realROI.toFixed(2)}%`,fin.realROI>=20?"#22c55e":"#f59e0b"],["Collection",`${fin.collectionRate.toFixed(1)}%`,"#f59e0b"]].map(([l,v,c])=>(<div key={l} style={{display:"flex",justifyContent:"space-between",marginBottom:6}}><span style={{fontSize:11,color:"#8ab4c8"}}>{l}</span><span style={{fontSize:12,fontWeight:700,color:c,fontFamily:"'Courier New',monospace"}}>{v}</span></div>))}</div>}
        <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginBottom:14}}>
          {[["On Time",fin.onTimeCount,"#4ade80","rgba(34,197,94,0.06)"],["Late",fin.lateCount,"#f59e0b","rgba(245,158,11,0.06)"],["Partial",fin.partialCount,"#f87171","rgba(239,68,68,0.06)"],["Overdue",fin.overdueCount,"#ef4444","rgba(239,68,68,0.08)"]].map(([l,v,c,bg])=>(<div key={l} style={{background:bg,borderRadius:10,padding:"10px 8px",textAlign:"center"}}><div style={{fontSize:16,fontWeight:800,color:c}}>{v}</div><div style={{fontSize:9,color:"#5a7a90",marginTop:2}}>{l}</div></div>))}
        </div>
        <div style={{background:`${pc}10`,border:`1px solid ${pc}30`,borderRadius:12,padding:"12px 14px",marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:11,color:pc,fontWeight:700,marginBottom:4}}>PERFORMANCE</div><div style={{display:"flex",gap:3}}>{[1,2,3,4,5].map(x=><span key={x} style={{color:x<=stars?"#f59e0b":"rgba(255,255,255,0.1)",fontSize:14}}>★</span>)}</div></div><div style={{fontSize:22,fontWeight:900,color:pc,fontFamily:"'Courier New',monospace"}}>{fin.completionRate.toFixed(0)}%</div></div>
        <button onClick={()=>setSchOpen(!schOpen)} style={{width:"100%",background:"rgba(100,180,255,0.06)",border:"1px solid rgba(100,180,255,0.12)",color:"#60a5fa",padding:"9px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",gap:6,marginBottom:schOpen?10:0}}><Icon name="calendar" size={13}/>{schOpen?"Hide":"View"} Schedule</button>
        {schOpen&&<div>{loan.schedule?.map((s,idx)=><InstRow key={idx} s={s} i={idx} loan={loan} isActive={isAct} isAdmin={isAdmin} today={today} onPay={onPay} onOverride={onOverride}/>)}</div>}
      </div>)}
    </div>
  );
}
...
function LoginScreen({users,onLogin}){
  ...
  const go=()=>{const f=users.find(x=>x.username===u.trim()&&x.password===p.trim()&&x.active);if(f){setErr("");onLogin(f);}else setErr("Invalid credentials.");};
  return(<div style={{minHeight:"100vh",background:"#060f1a",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}><div style={{width:"100%",maxWidth:380}}><div style={{textAlign:"center",marginBottom:32}}><div style={{width:64,height:64,borderRadius:18,background:"linear-gradient(135deg,#0a5c36,#0d7a48)",border:"2px solid rgba(200,146,10,0.4)",display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 16px"}}><svg width="34" height="34" viewBox="0 0 80 80" fill="none"><path d="M48 22A19 19 0 1 0 48 58" stroke="white" strokeWidth="5.5" strokeLinecap="round" fill="none"/><path d="M42 30L53 20L64 30" stroke="#c8920a" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" fill="none"/><line x1="53" y1="20" x2="53" y2="44" stroke="#c8920a" strokeWidth="4" strokeLinecap="round"/></svg></div><div style={{fontSize:28,fontWeight:900,color:"#e8f4fd",letterSpacing:2,fontFamily:"serif"}}>CREDA</div><div style={{fontSize:11,color:"rgba(200,146,10,0.8)",letterSpacing:3,textTransform:"uppercase",marginTop:4}}>Finance Platform</div></div><div style={{background:"#0d1b2a",border:"1px solid rgba(100,200,255,0.12)",borderRadius:20,padding:"28px 24px"}}><Field label="Username"><input style={IS} value={u} onChange={e=>setU(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Username" autoFocus/></Field><Field label="Password"><div style={{position:"relative"}}><input type={show?"text":"password"} style={{...IS,paddingRight:40}} value={p} onChange={e=>setP(e.target.value)} onKeyDown={e=>e.key==="Enter"&&go()} placeholder="Password"/><button onClick={()=>setShow(s=>!s)} style={{position:"absolute",right:10,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",color:"#3a5a70",cursor:"pointer"}}>{show?"🙈":"👁️"}</button></div></Field>{err&&<div style={{background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:8,padding:"10px 12px",fontSize:12,color:"#f87171",marginBottom:14}}>{err}</div>}<button onClick={go} style={{width:"100%",background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",padding:"13px",borderRadius:12,cursor:"pointer",fontWeight:800,fontSize:15}}>Sign In →</button></div></div></div>);
}
...
function StaffPanel({users,clients,pendingLoans,currentUser,onUpdateUsers,onApproveLoan,onRejectLoan,showToast}){
  ...
  const handleAdd=()=>{if(!ns.name||!ns.username||!ns.password){showToast("All fields required","error");return;}if(ns.password.length<4){showToast("Min 4 chars","error");return;}if(users.find(u=>u.username.toLowerCase()===ns.username.toLowerCase())){showToast("Username exists","error");return;}onUpdateUsers([...users,{...ns,id:generateId("USR"),createdAt:todayStr,active:true}]);setNs({name:"",username:"",password:"",role:"loan_officer"});setShowAdd(false);showToast("Account created!");};
  const handleReset=()=>{if(!newPwd||newPwd.length<4){showToast("Min 4 chars","error");return;}onUpdateUsers(users.map(u=>u.id===resetPwd.id?{...u,password:newPwd}:u));setResetPwd(null);setNewPwd("");showToast("Password reset!");};
  const getStats=id=>{const mc=clients.filter(c=>c.assignedTo===id);return{clients:mc.length,activeLoans:mc.reduce((a,c)=>a+(c.loans?.filter(l=>l.status==="active").length||0),0),collected:mc.reduce((a,c)=>a+(c.loans||[]).reduce((b,l)=>b+(l.schedule||[]).reduce((d,s)=>d+(s.paidAmount||0),0),0),0)};};

  // Staff monthly report data
  ...
  return(
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        <div><h1 style={{margin:0,fontSize:22,fontWeight:800,color:"#e8f4fd"}}>Staff</h1><p style={{margin:"4px 0 0",color:"#3a5a70",fontSize:13}}>{users.filter(u=>u.role!=="admin").length} members</p></div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setShowStaffReport(true)} style={{background:"rgba(167,139,250,0.12)",border:"1px solid rgba(167,139,250,0.25)",color:"#a78bfa",padding:"9px 14px",borderRadius:11,cursor:"pointer",fontWeight:700,fontSize:12,display:"flex",alignItems:"center",gap:6}}><Icon name="chart" size={13}/>Reports</button>
      ...
      {pendingLoans.length>0&&<div style={{background:"rgba(245,158,11,0.06)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:14,padding:16,marginBottom:20}}>
        ...
        {pendingLoans.map(pl=>{const off=users.find(u=>u.id===pl.requestedBy);const cl=clients.find(c=>c.id===pl.clientId);return(<div key={pl.id} style={{background:"rgba(0,0,0,0.2)",borderRadius:12,padding:"12px 14px",marginBottom:10}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><div><div style={{fontSize:13,fontWeight:700,color:"#e8f4fd"}}>{cl?.name||"?"}</div><div style={{fontSize:10,color:"#3a5a70"}}>By {off?.name} · {fd(pl.requestedAt?.split("T")[0])}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:14,fontWeight:800,color:"#60a5fa",fontFamily:"'Courier New',monospace"}}>{fc(pl.loanData.principal)}</div></div></div><div style={{display:"flex",gap:8}}><button onClick={()=>onApproveLoan(pl)} style={{flex:1,background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",padding:"9px",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:12}}>✓ Approve</button><button onClick={()=>onRejectLoan(pl.id)} style={{flex:1,background:"rgba(239,68,68,0.1)",border:"1px solid rgba(239,68,68,0.2)",color:"#f87171",padding:"9px",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:12}}>✕ Reject</button></div></div>);})}
      </div>}

      {users.filter(u=>u.role!=="admin").map(o=>{const st=getStats(o.id);return(<div key={o.id} style={{background:"rgba(255,255,255,0.025)",border:`1px solid ${o.active?"rgba(100,180,255,0.1)":"rgba(255,255,255,0.04)"}`,borderRadius:14,padding:"14px 16px",marginBottom:10}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:42,height:42,borderRadius:12,background:o.active?"linear-gradient(135deg,#1d4ed8,#3b82f6)":"rgba(255,255,255,0.06)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:17,color:o.active?"#fff":"#3a5a70"}}>{o.name.charAt(0)}</div><div><div style={{fontSize:14,fontWeight:700,color:o.active?"#dceef8":"#3a5a70"}}>{o.name}</div><div style={{fontSize:11,color:"#3a5a70"}}>@{o.username}</div><div style={{marginTop:4}}><RB role={o.role}/>{!o.active&&<Badge color="#6b7a8d" bg="rgba(107,122,141,0.1)"> INACTIVE</Badge>}</div></div></div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8}}>{[["Clients",st.clients,"#60a5fa"],["Active",st.activeLoans,"#4ade80"],["Collected",fc(st.collected),"#a78bfa"]].map(([l,v,c])=>(<div key={l} style={{background:"rgba(0,0,0,0.2)",borderRadius:9,padding:"9px 10px"}}><div style={{fontSize:9,color:"#3a5a70",marginBottom:3}}>{l}</div><div style={{fontSize:11,fontWeight:700,color:c,fontFamily:"'Courier New',monospace"}}>{v}</div></div>))}</div>
      </div>);})}

      {showAdd&&<Modal title="Create Staff Account" onClose={()=>setShowAdd(false)}>
        <div style={{background:"rgba(96,165,250,0.06)",border:"1px solid rgba(96,165,250,0.15)",borderRadius:10,padding:"10px 12px",marginBottom:14,fontSize:12,color:"#60a5fa"}}>👑 Only admin can create accounts. Staff can change their own password.</div>
      ...
      {editS&&<Modal title="Edit Staff" onClose={()=>setEditS(null)}>
        ...
        <button onClick={()=>{if(editS.username&&users.find(u=>u.username===editS.username&&u.id!==editS.id)){showToast("Taken","error");return;}onUpdateUsers(users.map(u=>u.id===editS.id?{...u,name:editS.name,username:editS.username}:u));setEditS(null);showToast("Updated!");}} style={{width:"100%",background:"linear-gradient(135deg,#3b82f6,#2563eb)",border:"none",color:"#fff",padding:"12px",borderRadius:12,cursor:"pointer",fontWeight:800}}>✓ Save</button>
      </Modal>}

      {resetPwd&&<Modal title="🔑 Reset Password" onClose={()=>{setResetPwd(null);setNewPwd("");}}>
        <div style={{background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:10,padding:"12px 14px",marginBottom:16}}>
          ...
          <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#1d4ed8,#3b82f6)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,color:"#fff",fontSize:15}}>{resetPwd.name.charAt(0)}</div><div><div style={{fontSize:14,fontWeight:700,color:"#e8f4fd"}}>{resetPwd.name}</div><div style={{fontSize:11,color:"#3a5a70"}}>@{resetPwd.username}</div></div></div>
        </div>
        ...
        <button onClick={handleReset} style={{width:"100%",background:"#f59e0b",border:"none",color:"#000",padding:"12px",borderRadius:12,cursor:"pointer",fontWeight:800}}>✓ Reset Password</button>
      </Modal>}

      {/* ── STAFF MONTHLY REPORT MODAL ──────────────────────────────────── */}
      {showStaffReport&&<Modal title="📊 Staff Monthly Report" onClose={()=>setShowStaffReport(false)} wide>
              ...
              <div key={sr.officer.id} style={{background:"rgba(255,255,255,0.025)",border:"1px solid rgba(100,180,255,0.1)",borderRadius:16,padding:"16px",marginBottom:14}}>
                ...
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:14}}>
                  ...
                  <div style={{textAlign:"right"}}>
                    ...
                    <div style={{fontSize:9,color:"#3a5a70"}}>Collection Rate</div>
                  </div>
                </div>
                ...
                <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:8,marginBottom:10}}>
                  {[
                    ...
                    ["👥 Total Clients",sr.totalClients,"#93c5fd"],
                    ["🏃 Active Clients",sr.activeClients,"#f59e0b"],
                    ["📋 Loans Issued",sr.loansIssued,"#c4b5fd"],
...
export default function App(){
  ...
  useEffect(()=>{
    const unsubs=[
      ...
      fbListen("users",v=>{if(!v||v.length===0){fbSet("users",DEFAULT_USERS);setUsers(DEFAULT_USERS);}else setUsers(v);}),
  ...
  const filteredClients=useMemo(()=>visibleClients.filter(c=>{
    ...
    if(clientFilter==="active")return match&&active;if(clientFilter==="completed")return match&&!active&&c.loans?.length>0;if(clientFilter==="none")return match&&(!c.loans||c.loans.length===0);return match;
  }),[visibleClients,clientSearch,clientFilter]);

  const stats=useMemo(()=>{
    const src=isAdmin?clients:visibleClients;
    if(isAdmin){let d=0,e=0,c=0,o=0;src.forEach(cl=>{(cl.loans||[]).forEach(l=>{d+=l.principal||0;e+=l.totalRepayable||0;(l.schedule||[]).forEach(s=>{if(s.paidAmount>0)c+=s.paidAmount;else if(new Date(s.dueDate)<today)o++;});});});return{totalDisbursed:d,totalExpected:e,totalCollected:c,outstanding:e-c,overdueCount:o,totalSavings:src.reduce((a,cl)=>a+(cl.savingsBalance||0),0)};}
    return computeMonthlyStats(src,curMonthKey);
  },[clients,visibleClients,isAdmin]);

  // ── MONTHLY REPORT DATA (ADMIN) ───────────────────────────────────────────
  const monthlyReport = useMemo(() => computeMonthlyReport(clients), [clients]);

  const globalTransactions=useMemo(()=>{const src=isAdmin?clients:visibleClients;const tx=[];src.forEach(c=>{(c.loans||[]).forEach(l=>{(l.schedule||[]).forEach(s=>{
    // Use paymentLog if available
    if(s.paymentLog&&s.paymentLog.length>0){
      s.paymentLog.forEach(pl=>{tx.push({clientId:c.id,clientName:c.name,day:s.day,totalDays:l.days,paidAmount:pl.amount,paidDate:pl.date,dueDate:s.dueDate,cascaded:pl.cascaded||false,by:pl.by||""});});
    } else if(s.paidAmount>0){tx.push({clientId:c.id,clientName:c.name,day:s.day,totalDays:l.days,paidAmount:s.paidAmount,paidDate:s.paidDate,dueDate:s.dueDate});}
  });});});return tx.sort((a,b)=>new Date(b.paidDate)-new Date(a.paidDate));},[clients,visibleClients,isAdmin]);

  const dailyCollectionData=useMemo(()=>{const src=isAdmin?clients:visibleClients;const map={};src.forEach(c=>{(c.loans||[]).forEach(l=>{(l.schedule||[]).forEach(s=>{const due=s.dueDate;if(!due)return;if(!map[due])map[due]={date:due,expected:0,collected:0,installments:[]};map[due].expected+=s.payment||0;map[due].collected+=s.paidAmount||0;map[due].installments.push({clientName:c.name,clientId:c.id,day:s.day,totalDays:l.days,payment:s.payment,paidAmount:s.paidAmount||0,paid:s.paid,paidDate:s.paidDate,dueDate:s.dueDate});});});});return Object.values(map).sort((a,b)=>new Date(b.date)-new Date(a.date));},[clients,visibleClients,isAdmin]);

  const computePP=(loan,idx,amt)=>{if(!amt||amt<=0)return null;const sim=applySmartPayment(loan.schedule,idx,amt,todayStr,currentUser?.name);return{daysCleared:sim.filter(s=>s.paid).length-loan.schedule.filter(s=>s.paid).length,remainingBal:sim[sim.length-1]?.balance||0};};
  const getClientSummary=c=>{const a=c.loans?.find(l=>l.status==="active");return{active:a,overdue:a?.schedule?.filter(s=>!s.paid&&new Date(s.dueDate)<today).length||0,paid:a?.schedule?.filter(s=>s.paid).length||0,total:a?.schedule?.length||0,balance:a?(a.schedule.find(s=>!s.paid)?.balance??0):0};};

  // ── ACTIONS ───────────────────────────────────────────────────────────────
  const handleAddClient=()=>{if(!newClient.name.trim()||!newClient.phone.trim())return;saveClients([{...newClient,id:generateId("CL"),loans:[],savingsBalance:0,savingsLogs:[],assignedTo:currentUser?.id,assignedToName:currentUser?.name,createdAt:todayStr},...clients]);setNewClient({name:"",phone:"",address:"",idNumber:""});setShowAddClient(false);showToast("Registered!");};
  ...
  const handleAddLoan=()=>{if(!newLoan.principal||!selectedClientId)return;const p=parseFloat(newLoan.principal),r=parseFloat(newLoan.interestRate),d=parseInt(newLoan.days);const calc=calcLoanSchedule(p,r,d,newLoan.startDate,newLoan.excludeWeekends);const loan={id:generateId("LN"),principal:p,interestRate:r,days:d,startDate:newLoan.startDate,...calc,status:"active",issuedAt:new Date().toISOString(),excludeWeekends:newLoan.excludeWeekends,issuedBy:currentUser?.id,issuedByName:currentUser?.name};saveClients(clients.map(c=>c.id===selectedClientId?{...c,loans:[...(c.loans||[]),loan]}:c));setNewLoan({principal:"",interestRate:"15",days:"30",startDate:todayStr,excludeWeekends:true});setShowAddLoan(false);showToast("Loan issued!");};
  const handleRequestLoan=()=>{if(!newLoan.principal||!selectedClientId)return;const p=parseFloat(newLoan.principal),r=parseFloat(newLoan.interestRate),d=parseInt(newLoan.days);const calc=calcLoanSchedule(p,r,d,newLoan.startDate,newLoan.excludeWeekends);savePending([...pendingLoans,{id:generateId("PL"),clientId:selectedClientId,requestedBy:currentUser?.id,requestedByName:currentUser?.name,requestedAt:new Date().toISOString(),loanData:{principal:p,interestRate:r,days:d,startDate:newLoan.startDate,...calc,excludeWeekends:newLoan.excludeWeekends}}]);setNewLoan({principal:"",interestRate:"15",days:"30",startDate:todayStr,excludeWeekends:true});setShowAddLoan(false);showToast("Request submitted!");};
  ...
  // ── PAYMENT (now includes paidBy in payment log) ──────────────────────────
  const handlePayment=()=>{
  ...
  const handleSavingsTransaction=()=>{const amt=parseFloat(savingsAmount);if(!amt||!showSavingsTx)return;const{type,client}=showSavingsTx;saveClients(clients.map(c=>{if(c.id!==client.id)return c;const cur=c.savingsBalance||0;if(type==="withdraw"&&amt>cur){showToast("Insufficient!","error");return c;}const nb=type==="deposit"?cur+amt:cur-amt;return{...c,savingsBalance:nb,savingsLogs:[{id:generateId("TX"),date:todayStr,type,amount:amt,balanceAfter:nb,recordedBy:currentUser?.name},...(c.savingsLogs||[])]};}));setSavingsAmount("");setShowSavingsTx(null);showToast(`${type==="deposit"?"Deposit":"Withdrawal"} done!`);};
  const handleAdminSavingsAdj=()=>{const nb=parseFloat(adminDirectSavingsInput);if(isNaN(nb)||!showSavingsTx)return;const{client}=showSavingsTx;saveClients(clients.map(c=>c.id!==client.id?c:{...c,savingsBalance:nb,savingsLogs:[{id:generateId("TX"),date:todayStr,type:"admin_adjustment",amount:Math.abs(nb-(c.savingsBalance||0)),balanceAfter:nb,recordedBy:currentUser?.name},...(c.savingsLogs||[])]}));setAdminDirectSavingsInput("");setShowSavingsTx(null);showToast("Adjusted!");};
  const handleAdminInstOverride=()=>{if(!adminEditInstallment)return;const{client,loan,idx}=adminEditInstallment;const pAmt=parseFloat(adminInstOverride.paidAmount)||0;saveClients(clients.map(c=>{if(c.id!==client.id)return c;return{...c,loans:c.loans.map(l=>{if(l.id!==loan.id)return l;const s=l.schedule.map((s,i)=>i===idx?{...s,paid:adminInstOverride.paid,dueDate:adminInstOverride.dueDate,paidDate:adminInstOverride.paid?(adminInstOverride.paidDate||todayStr):null,paidAmount:adminInstOverride.paid?pAmt:0,overpayment:adminInstOverride.paid&&pAmt>s.payment?pAmt-s.payment:0,shortfall:adminInstOverride.paid&&pAmt<s.payment?s.payment-pAmt:0,paymentLog:adminInstOverride.paid?[{amount:pAmt,date:adminInstOverride.paidDate||todayStr,by:currentUser?.name,at:new Date().toISOString()}]:[]}:s);let run=l.totalRepayable;const rc=s.map(s=>{if(s.paidAmount>0)run=Math.max(0,run-s.paidAmount);return{...s,balance:run};});return{...l,schedule:rc,status:rc.every(s=>s.paid)?"completed":"active"};})};};));setAdminEditInstallment(null);showToast("Override applied!");};
  ...
  const handleChangePassword=()=>{if(!profileEdit.oldPassword){showToast("Enter current password","error");return;}if(profileEdit.oldPassword!==currentUser.password){showToast("Wrong password","error");return;}if(!profileEdit.newPassword||profileEdit.newPassword.length<4){showToast("Min 4 chars","error");return;}if(profileEdit.newPassword!==profileEdit.confirmPassword){showToast("Don't match","error");return;}const upd={...currentUser,password:profileEdit.newPassword};saveUsers(users.map(u=>u.id===currentUser.id?upd:u));setCurrentUser(upd);setShowProfile(false);setProfileEdit({oldPassword:"",newPassword:"",confirmPassword:""});showToast("Password changed!");};
  const exportData=()=>{const blob=new Blob([JSON.stringify({clients,users,pendingLoans,v:"8.0",at:new Date().toISOString()},null,2)],{type:"application/json"});const url=URL.createObjectURL(blob),a=document.createElement("a");a.href=url;a.download=`creda_backup_${todayStr}.json`;a.click();URL.revokeObjectURL(url);showToast("Downloaded!");};

  if(loading)return(<div style={{minHeight:"100vh",background:"#060f1a",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16}}><div style={{width:56,height:56,borderRadius:16,background:"linear-gradient(135deg,#0a5c36,#0d7a48)",border:"2px solid rgba(200,146,10,0.4)",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="30" height="30" viewBox="0 0 80 80" fill="none"><path d="M48 22A19 19 0 1 0 48 58" stroke="white" strokeWidth="5.5" strokeLinecap="round" fill="none"/><path d="M42 30L53 20L64 30" stroke="#c8920a" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" fill="none"/><line x1="53" y1="20" x2="53" y2="44" stroke="#c8920a" strokeWidth="4" strokeLinecap="round"/></svg></div><div style={{color:"#e8f4fd",fontSize:16,fontWeight:700,fontFamily:"serif",letterSpacing:2}}>CREDA Finance</div><div style={{color:"#3a5a70",fontSize:12}}>Connecting…</div></div>);
  ...
  // ── VIEWS (abbreviated for space — same logic, key changes marked) ────────

  const Dashboard=()=>(<div>
    <div style={{marginBottom:22,display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
      <div><h1 style={{margin:0,fontSize:24,fontWeight:800,color:"#e8f4fd"}}>{isAdmin?"Dashboard":"My Dashboard"}</h1><p style={{margin:"4px 0 0",color:"#3a5a70",fontSize:13}}>{isAdmin?`${clients.length} clients`:`${visibleClients.length} clients · ${fm(curMonthKey)}`}</p></div>
      <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:6}}><RB role={currentUser.role}/>{isAdmin&&<button onClick={()=>setAdminMode("accountant")} style={{background:"rgba(167,139,250,0.15)",border:"1px solid rgba(167,139,250,0.3)",color:"#a78bfa",padding:"6px 12px",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:11}}>📊 Finance</button>}</div>
    </div>
    {!isAdmin&&<div style={{background:"rgba(96,165,250,0.08)",border:"1px solid rgba(96,165,250,0.2)",borderRadius:12,padding:"10px 14px",marginBottom:16,fontSize:12,color:"#60a5fa",fontWeight:700}}>📅 {fm(curMonthKey)} only — Resets next month</div>}
    ...
    {/* ── MONTHLY DISBURSEMENT REPORT (ADMIN ONLY) ────────────────────── */}
    {isAdmin&&<div style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(100,180,255,0.06)",borderRadius:14,padding:16,marginBottom:20}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><h3 style={{margin:0,fontSize:13,fontWeight:700,color:"#93c5fd",textTransform:"uppercase"}}>Monthly Disbursement Report</h3><p style={{margin:"2px 0 0",fontSize:11,color:"#3a5a70"}}>Loan issuance & collection by month</p></div>
      ...
      {expandMonthlyHistory&&<div style={{marginTop:16,display:"flex",flexDirection:"column",gap:10}}>
        {monthlyReport.length===0?<div style={{fontSize:11,color:"#3a5a70",textAlign:"center"}}>No data.</div>
          ...
          <div key={item.month} style={{padding:"12px 14px",background:"rgba(0,0,0,0.2)",borderRadius:12,borderLeft:"3px solid #22c55e"}}>
            ...
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr 1fr",gap:8}}>
              {[["DISBURSED",item.disbursed,"#93c5fd"],["COLLECTED",item.collected,"#4ade80"],["INTEREST",item.interest,"#c4b5fd"],["OUTSTANDING",item.outstanding,"#f87171"]].map(([l,v,c])=>(
                <div key={l}><div style={{fontSize:8,color:"#3a5a70"}}>{l}</div><div style={{fontSize:10,fontWeight:700,color:c,fontFamily:"'Courier New',monospace"}}>{fc(v)}</div></div>
              ))}
            </div>
            {/* Collection rate bar */}
            <div style={{marginTop:8}}>
              ...
              <div style={{height:4,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:`${item.expected>0?Math.min((item.collected/item.expected)*100,100):0}%`,background:"linear-gradient(90deg,#22c55e,#4ade80)",borderRadius:4}}/></div>
            </div>
          </div>
        ))}
      </div>}
  ...
  const ClientsList=()=>(<div>
    ...
    <div style={{position:"relative",marginBottom:10}}><span style={{position:"absolute",left:12,top:11,color:"#5a7a90"}}><Icon name="search" size={14}/></span><input style={{...IS,paddingLeft:36}} placeholder="Search…" value={clientSearch} onChange={e=>setClientSearch(e.target.value)}/></div>
  ...
  const AccountantView=()=>{
    ...
    const filteredDays=useMemo(()=>{if(acctDateFilter==="today")return dailyCollectionData.filter(d=>d.date===todayStr);if(acctDateFilter==="week"){const w=new Date(today);w.setDate(w.getDate()-7);return dailyCollectionData.filter(d=>new Date(d.date)>=w);}if(acctDateFilter==="month"){const m=new Date(today);m.setDate(m.getDate()-30);return dailyCollectionData.filter(d=>new Date(d.date)>=m);}return dailyCollectionData;},[acctDateFilter]);
    const totals=filteredDays.reduce((a,d)=>({expected:a.expected+d.expected,collected:a.collected+d.collected}),{expected:0,collected:0});
    return(<div>
      ...
      {filteredDays.map(day=>{const rate=day.expected>0?(day.collected/day.expected)*100:0;const isOpen=acctSelectedDay===day.date;return(<div key={day.date} style={{background:"rgba(255,255,255,0.02)",border:"1px solid rgba(100,180,255,0.08)",borderRadius:14,marginBottom:10,overflow:"hidden"}}><div onClick={()=>setAcctSelectedDay(isOpen?null:day.date)} style={{padding:"13px 15px",cursor:"pointer"}}><div style={{display:"flex",justifyContent:"space-between",marginBottom:8}}><div style={{fontSize:13,fontWeight:700,color:"#e8f4fd"}}>{fd(day.date)} {day.date===todayStr&&<Badge color="#4ade80" bg="rgba(34,197,94,0.15)">TODAY</Badge>}</div><div style={{fontSize:13,fontWeight:800,color:"#60a5fa",fontFamily:"'Courier New',monospace"}}>{fc(day.collected)}<span style={{color:"#3a5a70",fontWeight:400}}> / {fc(day.expected)}</span></div></div><div style={{height:5,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden"}}><div style={{height:"100%",width:`${Math.min(rate,100)}%`,background:"linear-gradient(90deg,#22c55e,#4ade80)",borderRadius:4}}/></div></div>{isOpen&&<div style={{padding:"10px 15px",borderTop:"1px solid rgba(255,255,255,0.05)"}}>{day.installments.map((inst,idx)=>(<div key={idx} onClick={()=>{setSelectedClientId(inst.clientId);setView("detail");setAcctSelectedDay(null);}} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid rgba(255,255,255,0.03)",cursor:"pointer"}}><div><div style={{fontSize:12,fontWeight:700,color:"#dceef8"}}>{inst.clientName}</div><div style={{fontSize:9,color:"#3a5a70"}}>Day {inst.day}/{inst.totalDays}</div></div><div style={{fontSize:12,fontWeight:700,color:inst.paid?"#4ade80":"#8ab4c8",fontFamily:"'Courier New',monospace"}}>{fc(inst.paid?inst.paidAmount:inst.payment)}</div></div>))}</div>}</div>);})}
    </div>);
  };

  const Detail=()=>{
    if(!selectedClient)return null;const c=selectedClient;const activeLoan=c.loans?.find(l=>l.status==="active");const allLoans=[...(c.loans||[])].reverse();
    ...
    return(<div>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
        ...
        <div style={{flexGrow:1}}><div style={{fontSize:18,fontWeight:800,color:"#e8f4fd"}}>{c.name}</div><div style={{fontSize:11,color:"#3a5a70"}}>{c.id}</div></div>
      ...
      <div style={{background:"rgba(100,180,255,0.04)",border:"1px solid rgba(100,180,255,0.08)",borderRadius:13,padding:"14px 16px",marginBottom:16,display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {[["PHONE",c.phone||"—"],["ID/BVN",c.idNumber||"—"],["SAVINGS",<span style={{color:"#c084fc",fontWeight:700}}>{fc(c.savingsBalance||0)}</span>],["OFFICER",c.assignedToName||"—"]].map(([l,v])=>(<div key={l}><div style={{fontSize:10,color:"#3a5a70",marginBottom:3}}>{l}</div><div style={{color:"#c8dde8",fontSize:13}}>{v}</div></div>))}
      ...
      <div style={{display:"flex",gap:3,marginBottom:16,background:"rgba(255,255,255,0.03)",borderRadius:10,padding:3}}>
        {[{id:"loans",label:`Cycles (${allLoans.length})`},{id:"history",label:"📋 History"},{id:"savings",label:"Savings"}].map(tab=>(<button key={tab.id} onClick={()=>setActiveTab(tab.id)} style={{flex:1,padding:"8px 4px",borderRadius:8,border:"none",cursor:"pointer",background:activeTab===tab.id?"rgba(100,180,255,0.1)":"transparent",color:activeTab===tab.id?(tab.id==="savings"?"#c084fc":tab.id==="history"?"#f59e0b":"#93c5fd"):"#3a5a70",fontWeight:600,fontSize:11}}>{tab.label}</button>))}
      </div>

      {activeTab==="loans"&&(allLoans.length===0?<div style={{textAlign:"center",padding:40,color:"#2a4050"}}>No cycles.</div>:<div>{allLoans.map((loan,idx)=>(<LCC key={loan.id} loan={loan} num={allLoans.length-idx} total={allLoans.length} isAdmin={isAdmin} today={today} onPay={(loan,si)=>{setShowPayment({clientId:c.id,loanId:loan.id,scheduleIdx:si});setPaymentAmount(loan.dailyPayment.toFixed(2));setPaymentPreview(null);}} onOverride={(loan,idx2,s)=>{setAdminEditInstallment({client:c,loan,idx:idx2});setAdminInstOverride({paid:s.paid,paidAmount:s.paidAmount||loan.dailyPayment.toFixed(2),dueDate:s.dueDate,paidDate:s.paidDate||todayStr});}}/>))}</div>)}
      ...
      {activeTab==="history"&&(<div>
        <div style={{background:"rgba(245,158,11,0.06)",border:"1px solid rgba(245,158,11,0.15)",borderRadius:12,padding:"12px 14px",marginBottom:14}}>
          <div style={{display:"flex",alignItems:"center",gap:8}}><Icon name="history" size={15}/><span style={{fontSize:14,fontWeight:700,color:"#f59e0b"}}>Transaction History</span></div>
          <div style={{fontSize:11,color:"#3a5a70",marginTop:4}}>{clientTx.length} transactions · Each payment shown separately with its date</div>
        </div>
        {clientTx.length===0?<div style={{textAlign:"center",padding:40,color:"#2a4050"}}>No transactions.</div>
        :clientTx.map(tx=>(<div key={tx.id} style={{background:tx.type==="loan_issued"?"rgba(96,165,250,0.04)":tx.type==="loan_payment"?"rgba(34,197,94,0.03)":"rgba(168,85,247,0.04)",border:`1px solid ${tx.color}25`,borderRadius:12,padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:8}}>
          <div style={{display:"flex",gap:10,alignItems:"flex-start"}}><div style={{width:32,height:32,borderRadius:8,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,background:"rgba(255,255,255,0.04)"}}>{tx.icon}</div><div><div style={{fontSize:13,fontWeight:700,color:"#e8f4fd"}}>{tx.description}</div><div style={{fontSize:10,color:"#3a5a70",marginTop:3}}>{tx.detail}</div><div style={{fontSize:9,color:"#3a5a70",marginTop:3}}>{fd(tx.date)}</div></div></div>
          <div style={{textAlign:"right",flexShrink:0}}><div style={{fontSize:14,fontWeight:800,color:tx.color,fontFamily:"'Courier New',monospace"}}>{tx.type==="loan_issued"||tx.type==="withdraw"?"−":"+"}{fc(tx.amount)}</div></div>
        </div>))}
      </div>)}
      ...
      {activeTab==="savings"&&<div>
        <div style={{background:"rgba(168,85,247,0.05)",border:"1px solid rgba(168,85,247,0.15)",borderRadius:14,padding:16,marginBottom:16}}>
          <div style={{textAlign:"center",marginBottom:14}}><div style={{fontSize:10,color:"#a855f7",fontWeight:700}}>BALANCE</div><div style={{fontSize:28,fontWeight:800,color:"#c084fc",fontFamily:"'Courier New',monospace",marginTop:6}}>{fc(c.savingsBalance||0)}</div></div>
        ...
        {(!c.savingsLogs||c.savingsLogs.length===0)?<div style={{textAlign:"center",padding:20,color:"#2a4050",fontSize:12}}>No history.</div>:c.savingsLogs.map((log,idx)=>(<div key={idx} style={{background:"rgba(255,255,255,0.01)",border:"1px solid rgba(168,85,247,0.08)",borderRadius:10,padding:"10px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6}}><div><div style={{fontSize:12,fontWeight:600,color:log.type==="deposit"?"#4ade80":log.type==="withdraw"?"#f87171":"#c084fc"}}>{log.type==="deposit"?"Deposit":log.type==="withdraw"?"Withdrawal":"Override"}</div><div style={{fontSize:9,color:"#3a5a70",marginTop:2}}>{fd(log.date)}{log.recordedBy&&` · ${log.recordedBy}`}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:12,fontWeight:700,color:"#e8f4fd",fontFamily:"'Courier New',monospace"}}>{log.type==="deposit"?"+":"-"}{fc(log.amount)}</div><div style={{fontSize:9,color:"#3a5a70"}}>Bal: {fc(log.balanceAfter)}</div></div></div>))}
      </div>}

      {isAdmin&&<button onClick={()=>setConfirmDelete(c.id)} style={{marginTop:22,background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.15)",color:"#f87171",padding:"9px 18px",borderRadius:10,cursor:"pointer",fontWeight:600,fontSize:12,display:"flex",alignItems:"center",gap:7}}><Icon name="trash" size={13}/>Delete</button>}
    </div>);
  };

  const navItems=[{id:"dashboard",label:"Overview",icon:"dashboard"},{id:"clients",label:isAdmin?"Clients":"Mine",icon:"user"},{id:"ledger",label:"Ledger",icon:"ledger"},...(isAdmin?[{id:"accountant",label:"Accounts",icon:"account"},{id:"staff",label:"Staff",icon:"users"}]:[])];

  return(<div style={{minHeight:"100vh",background:"#060f1a",fontFamily:"'Segoe UI',system-ui,sans-serif",color:"#e8f4fd"}}>
    ...
    <div style={{background:"rgba(6,15,26,0.95)",borderBottom:"1px solid rgba(100,180,255,0.07)",padding:"14px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:!isAdmin?28:0,zIndex:100,backdropFilter:"blur(10px)"}}>
      <div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#0a5c36,#0d7a48)",border:"1.5px solid rgba(200,146,10,0.4)",display:"flex",alignItems:"center",justifyContent:"center"}}><svg width="20" height="20" viewBox="0 0 80 80" fill="none"><path d="M48 22A19 19 0 1 0 48 58" stroke="white" strokeWidth="5.5" strokeLinecap="round" fill="none"/><path d="M42 30L53 20L64 30" stroke="#c8920a" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" fill="none"/><line x1="53" y1="20" x2="53" y2="44" stroke="#c8920a" strokeWidth="4" strokeLinecap="round"/></svg></div><div><div style={{fontSize:14,fontWeight:800,color:"#e8f4fd",letterSpacing:1,fontFamily:"serif"}}>CREDA</div></div></div>
      <div style={{display:"flex",alignItems:"center",gap:10}}>
    ...
    {showProfile&&<Modal title="🔑 Change Password" onClose={()=>setShowProfile(false)}>
      <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20,padding:12,background:"rgba(255,255,255,0.03)",borderRadius:12}}><div style={{width:50,height:50,borderRadius:14,background:isAdmin?"linear-gradient(135deg,#f59e0b,#d97706)":"linear-gradient(135deg,#1d4ed8,#3b82f6)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:22,color:"#fff"}}>{currentUser.name.charAt(0)}</div><div><div style={{fontWeight:700,color:"#e8f4fd",fontSize:16}}>{currentUser.name}</div><div style={{fontSize:12,color:"#3a5a70"}}>@{currentUser.username}</div></div></div>
      {!isAdmin&&<div style={{background:"rgba(96,165,250,0.06)",border:"1px solid rgba(96,165,250,0.15)",borderRadius:10,padding:"10px 12px",marginBottom:14,fontSize:12,color:"#60a5fa"}}>ℹ️ Contact admin to change username.</div>}
    ...
    {showPayment&&(()=>{const cl=clients.find(c=>c.id===showPayment.clientId);const ln=cl?.loans?.find(l=>l.id===showPayment.loanId);const amt=parseFloat(paymentAmount)||0;const exp=ln?.dailyPayment||0;const isOver=amt>exp,isUnder=amt>0&&amt<exp;return(<Modal title="Record Payment" onClose={()=>{setShowPayment(null);setPaymentAmount("");setPaymentPreview(null);}}>
      <div style={{textAlign:"center",marginBottom:16}}><div style={{fontSize:12,color:"#3a5a70"}}>Expected</div><div style={{fontSize:26,fontWeight:800,color:"#4ade80",fontFamily:"'Courier New',monospace"}}>{fc(exp)}</div><div style={{fontSize:11,color:"#3a5a70",marginTop:4}}>Day {showPayment.scheduleIdx+1} · {cl?.name}</div></div>
      <Field label="Amount (₦)"><input type="number" style={IS} value={paymentAmount} onChange={e=>{setPaymentAmount(e.target.value);const a=parseFloat(e.target.value)||0;if(a>0&&ln)setPaymentPreview(computePP(ln,showPayment.scheduleIdx,a));else setPaymentPreview(null);}} autoFocus/></Field>
      {paymentPreview&&amt>0&&<div style={{marginBottom:14}}>
    ...
    {adminEditInstallment&&<Modal title="🛠️ Override" onClose={()=>setAdminEditInstallment(null)}>
      ...
      {adminInstOverride.paid&&<><Field label="Amount"><input type="number" style={IS} value={adminInstOverride.paidAmount} onChange={e=>setAdminInstOverride(p=>({...p,paidAmount:e.target.value}))}/></Field><Field label="Date"><input type="date" style={IS} value={adminInstOverride.paidDate} onChange={e=>setAdminInstOverride(p=>({...p,paidDate:e.target.value}))}/></Field></>}
    ...
    {showSavingsTx&&<Modal title={showSavingsTx.type==="deposit"?"📥 Deposit":"📤 Withdraw"} onClose={()=>setShowSavingsTx(null)}>
      ...
      {isAdmin&&<div style={{borderTop:"1px dashed rgba(255,255,255,0.1)",paddingTop:16}}><div style={{fontSize:11,color:"#f59e0b",fontWeight:700,marginBottom:8}}>🛡️ Override</div><Field label="Set Balance"><input type="number" style={{...IS,border:"1px solid rgba(245,158,11,0.3)"}} value={adminDirectSavingsInput} onChange={e=>setAdminDirectSavingsInput(e.target.value)}/></Field><button onClick={handleAdminSavingsAdj} style={{width:"100%",background:"#f59e0b",border:"none",color:"#000",padding:10,borderRadius:10,cursor:"pointer",fontWeight:700}}>Force</button></div>}
    </Modal>}
    ...
    {showAddLoan&&<Modal title={isAdmin?"Issue Loan":"Request Loan"} onClose={()=>setShowAddLoan(false)}>
      ...
      {newLoan.principal&&(()=>{const p=parseFloat(newLoan.principal)||0,r=parseFloat(newLoan.interestRate)||0,d=parseInt(newLoan.days)||1,t=p+(p*r/100);return(<div style={{background:"rgba(34,197,94,0.06)",borderRadius:10,padding:12,marginBottom:14,display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}><div><div style={{fontSize:10,color:"#3a5a70"}}>TOTAL</div><div style={{color:"#4ade80",fontWeight:700,fontFamily:"'Courier New',monospace"}}>{fc(t)}</div></div><div><div style={{fontSize:10,color:"#3a5a70"}}>DAILY</div><div style={{color:"#60a5fa",fontWeight:700,fontFamily:"'Courier New',monospace"}}>{fc(t/d)}</div></div></div>);})()}
    ...
    {showSettings&&<Modal title="⚙️ Settings" onClose={()=>setShowSettings(false)}>
      <div style={{display:"flex",flexDirection:"column",gap:14}}>
        <div style={{background:"rgba(255,255,255,0.03)",borderRadius:12,padding:"14px 16px"}}><div style={{display:"flex",alignItems:"center",gap:10}}><div style={{width:40,height:40,borderRadius:11,background:isAdmin?"linear-gradient(135deg,#f59e0b,#d97706)":"linear-gradient(135deg,#1d4ed8,#3b82f6)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:16,color:"#fff"}}>{currentUser.name.charAt(0)}</div><div><div style={{fontSize:14,fontWeight:700,color:"#e8f4fd"}}>{currentUser.name}</div><div style={{fontSize:11,color:"#3a5a70"}}>@{currentUser.username}</div><RB role={currentUser.role}/></div></div></div>
