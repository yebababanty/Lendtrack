...
function getRepaymentDays(startDate, count, excludeWeekendsAndHolidays) {
  ...
  while (days.length < count) {
    ...
    if (excludeWeekendsAndHolidays) {
      if (day !== 0 && day !== 6 && !isHoliday(dateStr)) days.push(dateStr);
    } else {
      days.push(dateStr);
    }
    cursor.setDate(cursor.getDate() + 1);
...
function calcLoanSchedule(principal, interestRate, days, startDate, excludeWeekends) {
  ...
  for (let i = 0; i < days; i++) {
    ...
    schedule.push({
      ...
      balance, paid: false, paidDate: null, paidAmount: 0,
    });
  }
...
function computeFinancials(clients) {
  let totalCapital = 0;          // Sum of all principal disbursed
  let totalExpectedInterest = 0; // Sum of all interest charged
  let interestEarned = 0;        // Interest actually collected so far
  let principalCollected = 0;    // Principal portion already recovered
  let totalCollected = 0;        // All cash received (principal + interest)
  let outstandingPrincipal = 0;  // Principal not yet recovered
  let expenses = 0;              // Placeholder for future expense tracking

  clients.forEach(c => {
    (c.loans || []).forEach(l => {
      const principal = l.principal || 0;
      const interest = l.totalInterest || 0;
      const totalDue = l.totalRepayable || 0;
      const totalDays = l.days || 1;

      totalCapital += principal;
      totalExpectedInterest += interest;

      // Ratio of interest to total repayable
      const interestRatio = totalDue > 0 ? interest / totalDue : 0;
      const principalRatio = totalDue > 0 ? principal / totalDue : 0;

      (l.schedule || []).forEach(s => {
        if (s.paid) {
          const received = s.paidAmount || 0;
          totalCollected += received;
          // Apportion each payment between principal and interest
          interestEarned += received * interestRatio;
          principalCollected += received * principalRatio;
        }
      });
    });
  });

  outstandingPrincipal = totalCapital - principalCollected;

  // True Profit = interest actually earned (cash in hand from interest portion)
  const trueProfit = interestEarned;

  // Real ROI = (True Profit / Total Capital Deployed) × 100
  const realROI = totalCapital > 0 ? (trueProfit / totalCapital) * 100 : 0;

  // Capital At Risk = outstanding principal not yet recovered
  const capitalAtRisk = Math.max(0, outstandingPrincipal);

  // Collection Rate
  const collectionRate = (totalExpectedInterest + totalCapital) > 0
    ? (totalCollected / (totalExpectedInterest + totalCapital)) * 100
    : 0;
...
function computeClientFinancials(client) {
  let capital = 0, expectedInterest = 0, interestEarned = 0;
  let principalCollected = 0, totalCollected = 0;
  ...
  const trueProfit = interestEarned;
  const realROI = capital > 0 ? (trueProfit / capital) * 100 : 0;

  return { capital, expectedInterest, interestEarned, principalCollected, outstandingPrincipal, trueProfit, realROI, totalCollected };
}
...
function StatCard({ label, value, accent, sub }) {
  return (
    <div style={{
      ...
      borderRadius:14,padding:"16px 18px",borderTop:`2px solid ${accent}`,
    }}>
  ...
  return (
    <div style={{marginTop:4}}>
      ...
      <div style={{display:"flex", justifyContent:"space-between", marginTop:3}}>
        <span style={{fontSize:9, color:"#2a3a48"}}>0%</span>
        <span style={{fontSize:9, color:"#2a3a48"}}>100%</span>
      </div>
    </div>
  );
}
...
export default function App() {
  ...
  const [newLoan, setNewLoan] = useState({
    ...
    startDate: new Date().toISOString().split("T")[0], excludeWeekends: true
  });
  ...
  const monthlyHistory = useMemo(() => {
    ...
    clients.forEach(c => {
      (c.loans || []).forEach(l => {
        ...
        (l.schedule || []).forEach(s => {
          if (s.paid) map[key].collected += s.paidAmount || 0;
        });
      });
    });
    return Object.entries(map).map(([month, d]) => ({
      month, disbursed: d.disbursed, expected: d.expected,
      collected: d.collected, interest: d.interest,
  ...
  const stats = useMemo(() => {
    ...
    clients.forEach(c => {
      (c.loans || []).forEach(l => {
        ...
        totalExpected += l.totalRepayable || 0;
        (l.schedule || []).forEach(s => {
          if (s.paid) totalCollected += s.paidAmount || 0;
      ...
      (c.loans || []).forEach(l => {
        (l.schedule || []).forEach(s => {
          if (s.paid && s.paidAmount > 0) {
            txList.push({ clientId:c.id, clientName:c.name, loanId:l.id, day:s.day, totalDays:l.days, paidAmount:s.paidAmount, paidDate:s.paidDate, dueDate:s.dueDate });
          }
        });
      });
    });
  ...
  const filteredClients = useMemo(() => {
    return clients.filter(c => {
      ...
      if (clientFilter === "active") return matchSearch && active;
      if (clientFilter === "completed") return matchSearch && !active && hasLoans;
  ...
  const handleAddClient = () => {
    ...
    const client = { ...newClient, id:generateId("CL"), loans:[], savingsBalance:0, savingsLogs:[], createdAt:new Date().toISOString().split("T")[0] };
  ...
  const handlePayment = () => {
    ...
    const updated = clients.map(c => {
      ...
      return { ...c, loans: c.loans.map(l => {
        ...
        const newSched = l.schedule.map((s, i) => i === scheduleIdx ? { ...s, paid:true, paidDate:new Date().toISOString().split("T")[0], paidAmount:amount } : s);
        let running = l.totalRepayable;
        const recalced = newSched.map(s => { if (s.paid) running = Math.max(0, running - s.paidAmount); return { ...s, balance:running }; });
  ...
  const handleSavingsTransaction = () => {
    ...
    const updated = clients.map(c => {
      ...
      let newBal = type === "deposit" ? cur + amount : cur - amount;
      if (type === "withdraw" && amount > cur) { alert("⚠️ Insufficient savings balance!"); return c; }
  ...
  const handleAdminInstallmentOverride = () => {
    ...
    const updated = clients.map(c => {
      ...
      return { ...c, loans: c.loans.map(l => {
        ...
        const sched = l.schedule.map((s, i) => i === idx ? { ...s, paid:adminInstOverride.paid, dueDate:adminInstOverride.dueDate, paidDate:adminInstOverride.paid?(adminInstOverride.paidDate||new Date().toISOString().split("T")[0]):null, paidAmount:adminInstOverride.paid?pAmt:0 } : s);
        let running = l.totalRepayable;
        const recalced = sched.map(s => { if (s.paid) running = Math.max(0, running - s.paidAmount); return { ...s, balance:running }; });
        return { ...l, schedule:recalced, status:recalced.every(s=>s.paid)?"completed":"active" };
      })};
    });
  ...
  const FinancialPanel = ({ fin, isClient = false }) => {
      ...
      <div style={{
        ...
        <div style={{
          ...
          <div style={{display:"flex", alignItems:"center", gap:8}}>
            ...
            <div>
              <div style={{fontSize:13, fontWeight:800, color:"#e8f4fd", letterSpacing:0.3}}>
                {isClient ? "Client Financial Intelligence" : "Portfolio Intelligence Report"}
              </div>
              <div style={{fontSize:10, color:"#3a5a70", marginTop:1}}>
                {isClient ? "Per-client capital & return breakdown" : "Real-time portfolio metrics engine"}
              </div>
            </div>
          </div>
          {!isClient && (
            <button onClick={() => setExpandFinancials(!expandFinancials)} style={{
              background:"rgba(100,180,255,0.08)", border:"none", borderRadius:8,
              color:"#60a5fa", padding:"5px 12px", fontSize:11, cursor:"pointer", fontWeight:600,
            }}>
        ...
        {(isClient || expandFinancials) && (
          <div style={{padding:"14px 16px", display:"flex", flexDirection:"column", gap:8}}>
            ...
            <FinanceMetricRow
              ...
              sub="Sum of all principal issued"
              value={formatCurrency(fin.totalCapital ?? fin.capital)}
              valueColor="#60a5fa"
            ...
            <FinanceMetricRow
              ...
              sub="Capital not yet recovered from borrowers"
              value={formatCurrency(fin.outstandingPrincipal)}
              valueColor={fin.outstandingPrincipal > 0 ? "#f87171" : "#4ade80"}
              icon={<Icon name="shield" size={13}/>}
            />

            {/* Interest Earned */}
            <FinanceMetricRow
              label="Interest Earned (Collected)"
              sub="Interest portion of all received payments"
              value={formatCurrency(fin.interestEarned)}
              valueColor="#a78bfa"
              icon={<Icon name="trend" size={13}/>}
            />

            {/* Expected Interest */}
            <FinanceMetricRow
              label="Expected Total Interest"
              sub="Full interest if all loans complete"
              value={formatCurrency(fin.totalExpectedInterest ?? fin.expectedInterest)}
              valueColor="#c4b5fd"
              icon={<Icon name="trend" size={13}/>}
            />
            ...
            <div style={{
              ...
              <div>
                <div style={{fontSize:11, color:"#4ade80", fontWeight:700, letterSpacing:0.5}}>TRUE PROFIT (Cash Earned)</div>
                <div style={{fontSize:9, color:"#3a6050", marginTop:2}}>Interest actually received as real cash income</div>
              </div>
              <div style={{fontSize:18, fontWeight:900, color:"#22c55e", fontFamily:"'Courier New',monospace"}}>
                {formatCurrency(fin.trueProfit)}
              </div>
            </div>
            ...
            <div style={{
              ...
              borderRadius:10, border:`1px solid ${roiColor}33`,
            }}>
              <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8}}>
                <div>
                  <div style={{fontSize:11, color:roiColor, fontWeight:700, letterSpacing:0.5}}>REAL ROI %</div>
                ...
                <div style={{
                  ...
                  <span style={{fontSize:16, fontWeight:900, color:roiColor, fontFamily:"'Courier New',monospace"}}>
                    {fin.realROI.toFixed(2)}%
                  </span>
                </div>
              </div>
            ...
            {!isClient && (
              <div style={{
                ...
                <div>
                  <div style={{fontSize:14, fontWeight:800, color:"#f59e0b", fontFamily:"'Courier New',monospace", textAlign:"right"}}>
                    {fin.collectionRate.toFixed(1)}%
                  </div>
            ...
            <div style={{
              ...
              <div style={{
                padding:"10px 12px", background:"rgba(96,165,250,0.06)",
                ...
                <div style={{fontSize:12, fontWeight:700, color:"#60a5fa", fontFamily:"'Courier New',monospace"}}>
                  {formatCurrency(fin.principalCollected)}
                </div>
              </div>
              <div style={{
                padding:"10px 12px", background:"rgba(248,113,113,0.05)",
                borderRadius:10, border:"1px solid rgba(248,113,113,0.1)",
              }}>
                <div style={{fontSize:9, color:"#3a5a70", marginBottom:4}}>CAPITAL AT RISK</div>
                <div style={{fontSize:12, fontWeight:700, color:"#f87171", fontFamily:"'Courier New',monospace"}}>
                  {formatCurrency(fin.outstandingPrincipal)}
                </div>
              </div>
            </div>
    ...
    <div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20}}>
        ...
        <div style={{gridColumn:"span 2"}}>
          <StatCard label="Overdue Installments" value={stats.overdueCount + " late runs"} accent="#ef4444"/>
        </div>
      </div>

      {/* === FINANCIAL INTELLIGENCE PANEL === */}
      <FinancialPanel fin={globalFinancials}/>
      ...
      <div style={{
        ...
        <div style={{display:"flex", justifyContent:"space-between", alignItems:"center"}}>
          <div>
            <h3 style={{margin:0, fontSize:13, fontWeight:700, color:"#93c5fd", letterSpacing:0.5, textTransform:"uppercase"}}>Monthly Breakdown</h3>
            <p style={{margin:"2px 0 0", fontSize:11, color:"#3a5a70"}}>Historical monthly performance</p>
          </div>
          <button onClick={() => setExpandMonthlyHistory(!expandMonthlyHistory)} style={{
            ...
            {expandMonthlyHistory ? "Hide" : "View Months"}
          </button>
        </div>
        ...
        {expandMonthlyHistory && (
          <div style={{marginTop:16, display:"flex", flexDirection:"column", gap:10}}>
            {monthlyHistory.length === 0 ? (
              <div style={{fontSize:11, color:"#3a5a70", textAlign:"center", padding:"10px 0"}}>No data yet.</div>
              ...
              <div key={item.month} style={{
                ...
                <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr 1fr", gap:6}}>
                  {[
                    ["DISBURSED", item.disbursed, "#93c5fd"],
                    ["COLLECTED", item.collected, "#4ade80"],
                    ["INTEREST", item.interest, "#c4b5fd"],
                    ["OUTSTANDING", item.outstanding, "#f87171"],
                  ].map(([l, v, col]) => (
                    <div key={l}>
                      <div style={{fontSize:8, color:"#3a5a70"}}>{l}</div>
                      <div style={{fontSize:10, fontWeight:600, color:col, fontFamily:"'Courier New',monospace"}}>{formatCurrency(v)}</div>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {clients.length === 0 ? (
        <div style={{ border:"1px dashed rgba(100,180,255,0.1)", borderRadius:14, padding:40, textAlign:"center", color:"#2a4050" }}>
          ...
          <button onClick={() => { setView("clients"); setShowAddClient(true); }} style={{
            background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",
            padding:"9px 22px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:13,
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {clients.slice(0,5).map(c => {
            ...
            return (
              <div key={c.id} onClick={() => { setSelectedClientId(c.id); setView("detail"); }} style={{
    ...
    <div>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}>
        ...
        <button onClick={() => setShowAddClient(true)} style={{
          background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",
          padding:"9px 16px",borderRadius:11,cursor:"pointer",fontWeight:700,fontSize:13,
      ...
      <div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:16}}>
        <div style={{position:"relative"}}>
          ...
          <input type="text" style={{...inputStyle,paddingLeft:36}} placeholder="Search by name, ID or phone..."
              ...
              <div key={c.id} onClick={() => { setSelectedClientId(c.id); setView("detail"); }} style={{
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:active?10:0}}>
                  <div style={{display:"flex",gap:10,alignItems:"center"}}>
                    <div style={{
                      width:40,height:40,borderRadius:11,background:"linear-gradient(135deg,#1e3a5f,#2563eb)",
                      display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:16,color:"#93c5fd",flexShrink:0,
                    }}>{c.name.charAt(0).toUpperCase()}</div>
                    <div>
  ...
  const Ledger = () => (
    <div>
      <div style={{marginBottom:22}}>
        ...
        <p style={{margin:"4px 0 0",color:"#3a5a70",fontSize:13}}>All received daily payments in chronological order</p>
      </div>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          {globalTransactions.map((tx, idx) => (
            <div key={idx} style={{
              ...
              <div style={{textAlign:"right"}}>
                <div style={{fontSize:13,color:"#4ade80",fontWeight:700,fontFamily:"'Courier New',monospace"}}>+{formatCurrency(tx.paidAmount)}</div>
  ...
  const Detail = () => {
    ...
    const clientFin = useMemo(() => computeClientFinancials(c), [c]);
      ...
      <div>
        ...
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
          <button onClick={() => setView("clients")} style={{
            background:"rgba(255,255,255,0.06)",border:"none",color:"#8ab4c8",
            width:34,height:34,borderRadius:9,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",
          }}><Icon name="back" size={15}/></button>
          <div style={{
            width:44,height:44,borderRadius:12,background:"linear-gradient(135deg,#1d4ed8,#3b82f6)",
        ...
        {/* Client Financial Intelligence Panel */}
        <FinancialPanel fin={clientFin} isClient={true}/>

        {/* Issue Loan Button */}
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
          <div style={{fontSize:15,fontWeight:700,color:"#c8dde8"}}>Loan Operations</div>
          {!activeLoan && (
            <button onClick={() => setShowAddLoan(true)} style={{
              background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",
              padding:"7px 15px",borderRadius:9,cursor:"pointer",fontWeight:700,fontSize:12,
          ...
          <div>
            <div style={{
              ...
              <div style={{textAlign:"center"}}>
                <div style={{fontSize:10,color:"#a855f7",fontWeight:700,letterSpacing:1}}>SAVINGS BALANCE</div>
            ...
            <div style={{
              ...
              <div style={{height:5,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden"}}>
                <div style={{
                  ...
                  background:"linear-gradient(90deg,#22c55e,#4ade80)",transition:"width 0.5s",
                }}/>
              </div>
            </div>
              ...
              {activeLoan.schedule.map((s, i) => {
                ...
                return (
                  <div key={i} style={{
                    ...
                    borderRadius:11,padding:"10px 13px",display:"flex",justifyContent:"space-between",alignItems:"center",
                  }}>
                    <div style={{display:"flex",gap:10,alignItems:"center"}}>
                      <div style={{
                    ...
                    <div style={{textAlign:"right",flexShrink:0,display:"flex",alignItems:"center",gap:8}}>
                      <div>
                        <div style={{fontSize:9,color:"#3a5a70",textAlign:"right"}}>BALANCE</div>
                        <div style={{fontSize:11,color:"#e8f4fd",fontFamily:"'Courier New',monospace"}}>{formatCurrency(s.balance)}</div>
                      </div>
                      {!s.paid && (
                        <button onClick={() => { setShowPayment({clientId:c.id,loanId:activeLoan.id,scheduleIdx:i}); setPaymentAmount(activeLoan.dailyPayment.toFixed(2)); }} style={{
                          background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",
                      ...
                      {isAdmin && (
                        <button onClick={() => {
                          ...
                          setAdminInstOverride({paid:s.paid,paidAmount:s.paidAmount||activeLoan.dailyPayment.toFixed(2),dueDate:s.dueDate,paidDate:s.paidDate||new Date().toISOString().split("T")[0]});
                        }} style={{
              ...
              <div style={{display:"flex",flexDirection:"column",gap:7}}>
                {activeLoan.schedule.filter(s=>s.paid).map((s,i) => (
                  <div key={i} style={{
                    ...
                    borderRadius:11,padding:"11px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",
                  }}>
                    <div style={{display:"flex",gap:9,alignItems:"center"}}>
                      <div style={{width:28,height:28,borderRadius:7,background:"rgba(34,197,94,0.15)",display:"flex",alignItems:"center",justifyContent:"center"}}><Icon name="check" size={12}/></div>
                      <div>
        ...
        {!activeLoan && activeTab!=="savings" && (
          <div style={{border:"1px dashed rgba(100,180,255,0.08)",borderRadius:13,padding:40,textAlign:"center",color:"#2a4050"}}>
            <div style={{fontSize:32,marginBottom:10}}>💰</div>
            <div style={{fontSize:13}}>{completedLoans.length>0?`${completedLoans.length} completed loan(s). Issue a new one.`:"No active loan."}</div>
          </div>
        )}

        <button onClick={() => setConfirmDelete(c.id)} style={{
          marginTop:22,background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.15)",
          color:"#f87171",padding:"9px 18px",borderRadius:10,cursor:"pointer",fontWeight:600,fontSize:12,
  ...
  return (
    <div style={{minHeight:"100vh",background:"#060f1a",fontFamily:"'Segoe UI',system-ui,sans-serif",color:"#e8f4fd"}}>
      ...
      {isAdmin && (
        <div style={{background:"#f59e0b",color:"#000",padding:"4px 12px",fontSize:11,fontWeight:700,textAlign:"center",display:"flex",justifyContent:"center",gap:10,alignItems:"center",position:"sticky",top:0,zIndex:1000}}>
          🛡️ ADMIN OVERRIDES ACTIVE
          <button onClick={() => setIsAdmin(false)} style={{background:"rgba(0,0,0,0.15)",border:"none",borderRadius:4,color:"#000",fontSize:9,fontWeight:800,padding:"2px 6px",cursor:"pointer"}}>LOCK</button>
        </div>
      )}
      ...
      <div style={{
        ...
        position:"sticky",top:isAdmin?28:0,zIndex:100,backdropFilter:"blur(10px)",
      }}>
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          <div style={{
            width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#0a5c36,#0d7a48)",
            border:"1.5px solid rgba(200,146,10,0.4)",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,
          }}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10}}>
          ...
          <button onClick={() => isAdmin ? setIsAdmin(false) : setShowAdminLogin(true)} style={{
            ...
            color:isAdmin?"#f59e0b":"#8ab4c8",width:30,height:30,borderRadius:8,cursor:"pointer",
            display:"flex",alignItems:"center",justifyContent:"center",
          }}><Icon name={isAdmin?"unlock":"lock"} size={14}/></button>
      ...
      {/* Bottom Nav */}
      <div style={{
        position:"fixed",bottom:0,left:0,right:0,background:"rgba(6,15,26,0.97)",
        borderTop:"1px solid rgba(100,180,255,0.07)",display:"flex",backdropFilter:"blur(12px)",zIndex:900,
      }}>
        {[{id:"dashboard",label:"Overview",icon:"dashboard"},{id:"clients",label:"Clients",icon:"user"},{id:"ledger",label:"Ledger",icon:"ledger"}].map(n => {
          ...
          return (
            <button key={n.id} onClick={() => setView(n.id)} style={{
              ...
              color:active?"#22c55e":"#4a6880",display:"flex",flexDirection:"column",alignItems:"center",gap:4,
            }}>
      ...
      {showAdminLogin && (
        <Modal title="🛡️ Admin Access" onClose={() => setShowAdminLogin(false)}>
          <div style={{textAlign:"center",marginBottom:14,fontSize:13,color:"#8ab4c8"}}>Enter your security PIN to activate admin overrides.</div>
          <Field label="Security PIN">
            <input type="password" style={inputStyle} value={adminPinInput} onChange={e=>setAdminPinInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleAdminVerify()} placeholder="Enter PIN" autoFocus/>
          </Field>
          <button onClick={handleAdminVerify} style={{width:"100%",background:"linear-gradient(135deg,#f59e0b,#d97706)",border:"none",color:"#000",padding:"12px",borderRadius:12,cursor:"pointer",fontWeight:800,fontSize:14}}>✓ Unlock Admin</button>
        </Modal>
      )}

      {adminEditInstallment && (
        <Modal title="🛠️ Admin Override" onClose={() => setAdminEditInstallment(null)}>
          <div style={{background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:10,padding:12,fontSize:12,color:"#f59e0b",marginBottom:16}}>
            Overriding Installment {adminEditInstallment.idx+1} for <strong>{adminEditInstallment.client.name}</strong>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
            <label style={{fontSize:12,color:"#8ab4c8",fontWeight:600,flexGrow:1}}>Mark as Paid?</label>
            <input type="checkbox" style={{width:20,height:20,accentColor:"#22c55e",cursor:"pointer"}} checked={adminInstOverride.paid} onChange={e=>setAdminInstOverride(p=>({...p,paid:e.target.checked}))}/>
          </div>
          {adminInstOverride.paid && (<>
            <Field label="Amount Received (₦)"><input type="number" style={inputStyle} value={adminInstOverride.paidAmount} onChange={e=>setAdminInstOverride(p=>({...p,paidAmount:e.target.value}))}/></Field>
            <Field label="Received Date"><input type="date" style={inputStyle} value={adminInstOverride.paidDate} onChange={e=>setAdminInstOverride(p=>({...p,paidDate:e.target.value}))}/></Field>
          </>)}
      ...
      {showSavingsTx && (
        <Modal title={showSavingsTx.type==="deposit"?"📥 Deposit Savings":"📤 Withdraw Savings"} onClose={()=>setShowSavingsTx(null)}>
          <div style={{background:"rgba(168,85,247,0.08)",border:"1px solid rgba(168,85,247,0.15)",borderRadius:10,padding:"9px 13px",marginBottom:16,fontSize:13,color:"#c084fc"}}>
            Client: <strong>{showSavingsTx.client.name}</strong><br/>Balance: <strong>{formatCurrency(showSavingsTx.client.savingsBalance||0)}</strong>
          </div>
          ...
          <button onClick={handleSavingsTransaction} style={{width:"100%",background:"linear-gradient(135deg,#a855f7,#7c3aed)",border:"none",color:"#fff",padding:"12px",borderRadius:12,cursor:"pointer",fontWeight:800,fontSize:14,marginBottom:20}}>
            Confirm {showSavingsTx.type==="deposit"?"Deposit":"Withdrawal"}
          </button>
          {isAdmin && (
            <div style={{borderTop:"1px dashed rgba(255,255,255,0.1)",paddingTop:16}}>
              <div style={{fontSize:11,color:"#f59e0b",fontWeight:700,marginBottom:8}}>🛡️ Admin: Direct Balance Override</div>
              <Field label="Set Balance To (₦)"><input type="number" style={{...inputStyle,border:"1px solid rgba(245,158,11,0.3)"}} value={adminDirectSavingsInput} onChange={e=>setAdminDirectSavingsInput(e.target.value)} placeholder="New absolute balance"/></Field>
              <button onClick={handleAdminSavingsAdjustment} style={{width:"100%",background:"#f59e0b",border:"none",color:"#000",padding:"10px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:12}}>Force Adjust Balance</button>
            </div>
          )}
        </Modal>
      )}
      ...
      {showAddLoan && (
        <Modal title="Issue New Loan" onClose={()=>setShowAddLoan(false)}>
          <div style={{background:"rgba(59,130,246,0.08)",border:"1px solid rgba(59,130,246,0.15)",borderRadius:9,padding:"9px 13px",marginBottom:16,fontSize:13,color:"#93c5fd"}}>
          ...
          <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 0 16px"}}>
            <span style={{fontSize:12,color:"#8ab4c8",fontWeight:600}}>Skip Weekends & Public Holidays?</span>
            <input type="checkbox" style={{width:20,height:20,accentColor:"#22c55e",cursor:"pointer"}} checked={newLoan.excludeWeekends} onChange={e=>setNewLoan(p=>({...p,excludeWeekends:e.target.checked}))}/>
          </div>
          {newLoan.principal && (() => {
            const p=parseFloat(newLoan.principal)||0, r=parseFloat(newLoan.interestRate)||0, d=parseInt(newLoan.days)||1;
            const total=p+(p*r/100), daily=total/d;
            return (
              <div style={{background:"rgba(34,197,94,0.06)",border:"1px solid rgba(34,197,94,0.12)",borderRadius:10,padding:"12px 14px",marginBottom:14,display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
                ...
                <div><div style={{fontSize:10,color:"#3a5a70",marginBottom:3}}>INTEREST</div><div style={{color:"#c4b5fd",fontWeight:700,fontFamily:"'Courier New',monospace",fontSize:13}}>{formatCurrency(p*r/100)}</div></div>
                <div><div style={{fontSize:10,color:"#3a5a70",marginBottom:3}}>REAL ROI (this loan)</div><div style={{color:"#f59e0b",fontWeight:700,fontFamily:"'Courier New',monospace",fontSize:13}}>{r.toFixed(1)}%</div></div>
              </div>
            );
          })()}
          <button onClick={handleAddLoan} style={{width:"100%",background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",padding:"12px",borderRadius:12,cursor:"pointer",fontWeight:800,fontSize:14}}>✓ Issue Loan</button>
        </Modal>
      )}
        ...
        return (
          <Modal title="Record Payment" onClose={()=>{setShowPayment(null);setPaymentAmount("");}}>
            <div style={{textAlign:"center",marginBottom:20}}>
              ...
              <div style={{fontSize:26,fontWeight:800,color:"#4ade80",fontFamily:"'Courier New',monospace"}}>{formatCurrency(ln?.dailyPayment||0)}</div>
              <div style={{fontSize:11,color:"#3a5a70",marginTop:4}}>Day {showPayment.scheduleIdx+1} of {ln?.days}</div>
            </div>
            <Field label="Amount Received (₦)">
              <input type="number" style={inputStyle} value={paymentAmount} onChange={e=>setPaymentAmount(e.target.value)} placeholder="Enter amount" autoFocus/>
            </Field>
            <button onClick={handlePayment} style={{width:"100%",background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",padding:"12px",borderRadius:12,cursor:"pointer",fontWeight:800,fontSize:14,marginTop:6}}>✓ Confirm Payment</button>
          </Modal>
        );
      })()}
        ...
        <Modal title="⚙️ Settings & Backup" onClose={()=>setShowSettings(false)}>
          <div style={{display:"flex",flexDirection:"column",gap:14}}>
            <div style={{background:"rgba(34,197,94,0.06)",border:"1px solid rgba(34,197,94,0.15)",borderRadius:10,padding:"12px 14px"}}>
              <div style={{fontSize:12,color:"#4ade80",fontWeight:700,marginBottom:4}}>💾 Local Storage</div>
              <div style={{fontSize:12,color:"#6b7a8d",lineHeight:1.5}}>Data is saved on this device. Export backups regularly for safety.</div>
            </div>
            <div>
              <div style={{fontSize:12,color:"#8a9bb0",marginBottom:8,fontWeight:600}}>📤 EXPORT</div>
              <button onClick={()=>{exportData(clients);setShowSettings(false);}} style={{width:"100%",background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",padding:"11px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:13}}>Download Backup (.json)</button>
            </div>
            <div>
              <div style={{fontSize:12,color:"#8a9bb0",marginBottom:8,fontWeight:600}}>📥 RESTORE</div>
              <label style={{display:"block",width:"100%",background:"rgba(96,165,250,0.1)",border:"1px solid rgba(96,165,250,0.25)",color:"#60a5fa",padding:"11px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:13,textAlign:"center",boxSizing:"border-box"}}>
                Choose Backup File
                <input type="file" accept=".json" style={{display:"none"}} onChange={e=>{if(e.target.files[0]){importData(e.target.files[0],data=>{setClients(data);setShowSettings(false);alert("✅ "+data.length+" clients restored.");});}}}/>
              </label>
            </div>
            <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:14}}>
              ...
              <div style={{background:"rgba(255,255,255,0.03)",borderRadius:8,padding:"10px 12px",display:"flex",flexDirection:"column",gap:6}}>
                {[["Clients",clients.length,"#fff"],["Active Loans",clients.reduce((a,c)=>a+(c.loans?.filter(l=>l.status==="active").length||0),0),"#4ade80"],["Data Size",(JSON.stringify(clients).length/1024).toFixed(1)+" KB","#60a5fa"]].map(([l,v,col])=>(
                  <div key={l} style={{display:"flex",justifyContent:"space-between",fontSize:12,color:"#6b7a8d"}}>
      ...
      {confirmDelete && (
        <Modal title="Delete Client?" onClose={()=>setConfirmDelete(null)}>
          <p style={{color:"#8ab4c8",fontSize:14,marginBottom:20,lineHeight:1.6}}>This will permanently delete the client and all records. This cannot be undone.</p>
