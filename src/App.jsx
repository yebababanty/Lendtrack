  ...
  return (
    <div style={{ marginTop:4 }}>
      ...
      <div style={{ height:6,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden" }}>
        <div style={{ height:"100%",width:`${capped}%`,background:`linear-gradient(90deg,${color},${color}cc)`,borderRadius:4 }}/>
      </div>
    </div>
  );
}
    ...
    <div style={{ background:"linear-gradient(135deg,rgba(16,30,50,0.9),rgba(10,20,40,0.95))",border:"1px solid rgba(100,180,255,0.1)",borderRadius:16,overflow:"hidden",marginBottom:20 }}>
      <div style={{ background:"linear-gradient(135deg,rgba(59,130,246,0.12),rgba(34,197,94,0.08))",padding:"14px 16px",borderBottom:"1px solid rgba(100,180,255,0.08)",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        ...
        {!isClient && setExpandFinancials && (
          <button onClick={()=>setExpandFinancials(!expandFinancials)} style={{ background:"rgba(100,180,255,0.08)",border:"none",borderRadius:8,color:"#60a5fa",padding:"5px 12px",fontSize:11,cursor:"pointer",fontWeight:600 }}>{expandFinancials?"Collapse":"Expand"}</button>
        )}
      </div>
      ...
      <div onClick={()=>setExpanded(!expanded)} style={{ padding:"14px 16px",cursor:"pointer",borderBottom:expanded?"1px solid rgba(255,255,255,0.05)":"none" }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
          <div style={{ display:"flex",alignItems:"center",gap:10 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:10,color:"#3a5a70" }}>{formatDate(loan.startDate)} · {loan.days}d · {loan.interestRate}% · {loan.excludeWeekends?"Work days":"All days"}</div>
            </div>
          </div>
          <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            <div style={{ textAlign:"right" }}>
        ...
        <div style={{ marginTop:10 }}>
          ...
          <div style={{ height:4,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden" }}>
            <div style={{ height:"100%",width:`${fin.completionRate}%`,background:`linear-gradient(90deg,${perfColor},${perfColor}99)`,borderRadius:4 }}/>
          </div>
        </div>
      ...
      {expanded && (
        <div style={{ padding:"14px 16px" }}>
          ...
          <div style={{ background:`linear-gradient(135deg,${perfColor}10,${perfColor}05)`,border:`1px solid ${perfColor}30`,borderRadius:12,padding:"12px 14px",marginBottom:14,display:"flex",justifyContent:"space-between",alignItems:"center" }}>
            <div>
              <div style={{ fontSize:11,color:perfColor,fontWeight:700,marginBottom:4 }}>PERFORMANCE</div>
          ...
          <button onClick={()=>setScheduleOpen(!scheduleOpen)} style={{ width:"100%",background:"rgba(100,180,255,0.06)",border:"1px solid rgba(100,180,255,0.12)",color:"#60a5fa",padding:"9px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",gap:6,marginBottom:scheduleOpen?10:0 }}>
            <Icon name="calendar" size={13}/>{scheduleOpen?"Hide":"View"} Schedule {scheduleOpen?<Icon name="chevronUp" size={13}/>:<Icon name="chevronDown" size={13}/>}
          </button>
          {scheduleOpen && (
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#060f1a",display:"flex",alignItems:"center",justifyContent:"center",padding:20 }}>
      <div style={{ width:"100%",maxWidth:380 }}>
        ...
        <div style={{ background:"#0d1b2a",border:"1px solid rgba(100,200,255,0.12)",borderRadius:20,padding:"28px 24px",boxShadow:"0 40px 100px rgba(0,0,0,0.5)" }}>
          ...
          <Field label="Password">
            <div style={{ position:"relative" }}>
              <input type={show?"text":"password"} style={{ ...inputStyle,paddingRight:40 }} value={pwd} onChange={e=>setPwd(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleLogin()} placeholder="Enter password"/>
    ...
    <div>
      ...
      {pendingLoans.length > 0 && (
        <div style={{ background:"rgba(245,158,11,0.06)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:14,padding:16,marginBottom:20 }}>
          <div style={{ display:"flex",alignItems:"center",gap:8,marginBottom:12 }}><Icon name="bell" size={16}/><span style={{ fontSize:14,fontWeight:700,color:"#f59e0b" }}>Pending Approvals ({pendingLoans.length})</span></div>
          {pendingLoans.map(pl => {
            ...
            return (
              <div key={pl.id} style={{ background:"rgba(0,0,0,0.2)",borderRadius:12,padding:"12px 14px",marginBottom:10 }}>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6,marginBottom:10 }}>
                  {[["TOTAL",formatCurrency(pl.loanData.totalRepayable),"#4ade80"],["DAILY",formatCurrency(pl.loanData.dailyPayment),"#60a5fa"],["INTEREST",formatCurrency(pl.loanData.totalInterest),"#c4b5fd"]].map(([l,v,c])=>(
                    <div key={l} style={{ background:"rgba(255,255,255,0.03)",borderRadius:7,padding:"7px 9px" }}><div style={{ color:"#3a5a70",fontSize:8,marginBottom:2 }}>{l}</div><div style={{ color:c,fontWeight:700,fontFamily:"'Courier New',monospace",fontSize:10 }}>{v}</div></div>
                  ))}
                </div>
        ...
        return (
          <div key={o.id} style={{ background:"rgba(255,255,255,0.025)",border:`1px solid ${o.active?"rgba(100,180,255,0.1)":"rgba(255,255,255,0.04)"}`,borderRadius:14,padding:"14px 16px",marginBottom:10 }}>
            ...
            <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8 }}>
              {[["Clients",st.clients,"#60a5fa"],["Active",st.activeLoans,"#4ade80"],["Collected",formatCurrency(st.collected),"#a78bfa"]].map(([l,v,c])=>(
                <div key={l} style={{ background:"rgba(0,0,0,0.2)",borderRadius:9,padding:"9px 10px" }}><div style={{ fontSize:9,color:"#3a5a70",marginBottom:3 }}>{l}</div><div style={{ fontSize:11,fontWeight:700,color:c,fontFamily:"'Courier New',monospace" }}>{v}</div></div>
              ))}
            </div>
          </div>
        );
      ...
      {showAdd && (
        <Modal title="Add Staff" onClose={()=>setShowAdd(false)}>
          <Field label="Full Name *"><input style={inputStyle} value={ns.name} onChange={e=>setNs(p=>({...p,name:e.target.value}))} autoFocus/></Field>
          ...
          <Field label="Role"><select style={inputStyle} value={ns.role} onChange={e=>setNs(p=>({...p,role:e.target.value}))}><option value="loan_officer">Loan Officer</option><option value="accountant">Accountant</option></select></Field>
      ...
      {editS && (
        <Modal title="Edit Staff" onClose={()=>setEditS(null)}>
          ...
          <Field label="New Password (blank = keep)"><input type="password" style={inputStyle} placeholder="Leave blank to keep" onChange={e=>{ if(e.target.value) setEditS(p=>({...p,password:e.target.value})); }}/></Field>
...
export default function App() {
  ...
  const [adminMode, setAdminMode] = useState("admin"); // "admin" | "accountant"
  ...
  const handleAddClient = () => {
    ...
    const c = { ...newClient,id:generateId("CL"),loans:[],savingsBalance:0,savingsLogs:[],assignedTo:currentUser?.id,assignedToName:currentUser?.name,createdAt:todayStr };
  ...
  const handleUpdateClient = () => {
    ...
    setShowEditClient(null); showToast("Client updated!");
  };
  ...
  const handleAddLoan = () => {
    ...
    setNewLoan({ principal:"",interestRate:"15",days:"30",startDate:todayStr,excludeWeekends:true }); setShowAddLoan(false);
  ...
  const handlePayment = () => {
    ...
    saveClients(updated); setPaymentAmount(""); setShowPayment(null); setPaymentPreview(null);
  ...
  const handleSavingsTransaction = () => {
    ...
    const updated = clients.map(c=>{
      ...
      if (type==="withdraw"&&amount>cur) { showToast("Insufficient savings!","error"); return c; }
      const newBal = type==="deposit"?cur+amount:cur-amount;
      const log = { id:generateId("TX"),date:todayStr,type,amount,balanceAfter:newBal,recordedBy:currentUser?.name };
      return { ...c,savingsBalance:newBal,savingsLogs:[log,...(c.savingsLogs||[])] };
    });
    saveClients(updated); setSavingsAmount(""); setShowSavingsTx(null);
    showToast(type==="deposit"?"Deposit recorded!":"Withdrawal recorded!");
  };
  ...
  const handleAdminSavingsAdjustment = () => {
    ...
    const updated = clients.map(c=>{
      if (c.id!==client.id) return c;
      const log = { id:generateId("TX"),date:todayStr,type:"admin_adjustment",amount:Math.abs(newBal-(c.savingsBalance||0)),balanceAfter:newBal,recordedBy:currentUser?.name };
      return { ...c,savingsBalance:newBal,savingsLogs:[log,...(c.savingsLogs||[])] };
    });
    saveClients(updated); setAdminDirectSavingsInput(""); setShowSavingsTx(null);
  ...
  const handleAdminInstallmentOverride = () => {
    ...
    const updated = clients.map(c=>{
      ...
      return { ...c,loans:c.loans.map(l=>{
        ...
        const rc = s.map(s=>{ if(s.paidAmount>0)run=Math.max(0,run-s.paidAmount); return{...s,balance:run}; });
        return { ...l,schedule:rc,status:rc.every(s=>s.paid)?"completed":"active" };
      })};
    });
    saveClients(updated); setAdminEditInstallment(null);
  ...
  const handleAssignClient = (clientId, staffId) => {
    ...
    setShowAssignClient(null); showToast("Client reassigned!");
  };

  const handleDeleteClient = (id) => {
    saveClients(clients.filter(c=>c.id!==id));
    setConfirmDelete(null); setView("clients"); showToast("Client deleted","error");
  };

  const handleUpdateProfile = () => {
    if (!profileEdit.oldPassword) { showToast("Enter current password","error"); return; }
    if (profileEdit.oldPassword !== currentUser.password) { showToast("Wrong current password","error"); return; }
    ...
    setCurrentUser(updated); setShowProfile(false);
  ...
  const AccountantView = () => {
    ...
    return (
      <div>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20 }}>
          <div><h1 style={{ margin:0,fontSize:22,fontWeight:800,color:"#e8f4fd" }}>Accountant</h1><p style={{ margin:"4px 0 0",color:"#3a5a70",fontSize:13 }}>Daily collection schedule & performance</p></div>
          ...
          {todayData ? (
            <>
              <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:12 }}>
                {[["Expected",todayData.expected,"#60a5fa"],["Collected",todayData.collected,"#4ade80"],["Shortfall",Math.max(0,todayData.expected-todayData.collected),todayData.collected>=todayData.expected?"#4ade80":"#f87171"]].map(([l,v,c])=>(
                  <div key={l} style={{ background:"rgba(0,0,0,0.25)",borderRadius:10,padding:"10px 11px" }}>
                    ...
                    <div style={{ fontSize:12,fontWeight:800,color:c,fontFamily:"'Courier New',monospace" }}>{formatCurrency(v)}</div>
                  </div>
                ))}
              </div>
              <div>
                <div style={{ display:"flex",justifyContent:"space-between",fontSize:10,color:"#3a5a70",marginBottom:5 }}>
                  <span>{todayData.installments.filter(i=>i.paid).length}/{todayData.installments.length} paid</span>
                  <span style={{ color:todayData.collected>=todayData.expected?"#4ade80":"#f59e0b" }}>{todayData.expected>0?((todayData.collected/todayData.expected)*100).toFixed(1):0}% collected</span>
                </div>
                <div style={{ height:6,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden" }}>
                  <div style={{ height:"100%",width:`${todayData.expected>0?Math.min((todayData.collected/todayData.expected)*100,100):0}%`,background:"linear-gradient(90deg,#22c55e,#4ade80)",borderRadius:4 }}/>
                </div>
              </div>
            </>
        ...
        <div style={{ background:"rgba(255,255,255,0.02)",border:"1px solid rgba(100,180,255,0.07)",borderRadius:14,padding:"14px 16px",marginBottom:16 }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:10 }}>
            {[["Total Expected",totals.expected,"#60a5fa","rgba(96,165,250,0.06)","rgba(96,165,250,0.12)"],["Total Collected",totals.collected,"#4ade80","rgba(34,197,94,0.06)","rgba(34,197,94,0.12)"],["Outstanding",Math.max(0,totals.expected-totals.collected),"#f87171","rgba(239,68,68,0.05)","rgba(239,68,68,0.1)"],["Rate",`${totals.expected>0?((totals.collected/totals.expected)*100).toFixed(1):0}%`,"#f59e0b","rgba(245,158,11,0.05)","rgba(245,158,11,0.1)"]].map(([l,v,c,bg,border])=>(
              <div key={l} style={{ background:bg,borderRadius:10,padding:"11px 13px",border:`1px solid ${border}` }}>
                <div style={{ fontSize:9,color:"#3a5a70",marginBottom:4,textTransform:"uppercase" }}>{l}</div>
                <div style={{ fontSize:14,fontWeight:800,color:c,fontFamily:"'Courier New',monospace" }}>{typeof v==="string"?v:formatCurrency(v)}</div>
              </div>
            ))}
          </div>
          <div style={{ height:5,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden" }}>
            <div style={{ height:"100%",width:`${totals.expected>0?Math.min((totals.collected/totals.expected)*100,100):0}%`,background:"linear-gradient(90deg,#f59e0b,#fbbf24)",borderRadius:4 }}/>
          </div>
        </div>

        {/* Filter Tabs */}
        <div style={{ display:"flex",gap:6,marginBottom:16,overflowX:"auto",paddingBottom:4 }}>
          {[{id:"today",l:"Today"},{id:"week",l:"Week"},{id:"month",l:"30 Days"},{id:"all",l:"All"}].map(f=>(
        ...
        {filteredDays.length === 0
          ...
          : filteredDays.map(day => {
            const isToday2 = day.date === todayStr;
            const isPast = new Date(day.date) < today && !isToday2;
            ...
            if (isToday2){borderColor="rgba(34,197,94,0.25)";accentColor="#22c55e";}
            else if(rate>=100){borderColor="rgba(34,197,94,0.2)";accentColor="#22c55e";}
            else if(rate>0){borderColor="rgba(245,158,11,0.2)";accentColor="#f59e0b";}
            else if(isPast){borderColor="rgba(239,68,68,0.2)";accentColor="#ef4444";}
            return (
              <div key={day.date} style={{ background:isToday2?"linear-gradient(135deg,rgba(34,197,94,0.05),rgba(59,130,246,0.03))":"rgba(255,255,255,0.02)",border:`1px solid ${borderColor}`,borderRadius:14,overflow:"hidden",marginBottom:10 }}>
                <div onClick={()=>setAcctSelectedDay(isOpen?null:day.date)} style={{ padding:"13px 15px",cursor:"pointer" }}>
                    ...
                    <div style={{ display:"flex",alignItems:"center",gap:9 }}>
                      ...
                      <div>
                        <div style={{ fontSize:13,fontWeight:700,color:"#e8f4fd",display:"flex",alignItems:"center",gap:6,flexWrap:"wrap" }}>
                          {formatDate(day.date)}
                          {isToday2&&<Badge color="#4ade80" bg="rgba(34,197,94,0.15)">TODAY</Badge>}
                  ...
                  <div>
                    <div style={{ display:"flex",justifyContent:"space-between",fontSize:9,color:"#3a5a70",marginBottom:4 }}>
                      <span>{rate>=100?"✓ Fully Collected":rate>0?"Partial":isPast?"⚠ Not Collected":"Pending"}</span>
                      <span style={{ color:accentColor,fontWeight:700 }}>{rate.toFixed(1)}%</span>
                    </div>
                    <div style={{ height:5,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden" }}>
                      <div style={{ height:"100%",width:`${Math.min(rate,100)}%`,background:`linear-gradient(90deg,${accentColor},${accentColor}99)`,borderRadius:4 }}/>
                    </div>
                  </div>
                </div>
                {isOpen && (
                  <div style={{ borderTop:"1px solid rgba(255,255,255,0.05)",padding:"12px 15px" }}>
                    <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,marginBottom:12 }}>
                      {[["Expected",day.expected,"#60a5fa"],["Collected",day.collected,"#4ade80"],["Balance",Math.max(0,day.expected-day.collected),day.collected>=day.expected?"#4ade80":"#f87171"]].map(([l,v,c])=>(
                        <div key={l} style={{ background:"rgba(0,0,0,0.2)",borderRadius:9,padding:"9px 10px" }}><div style={{ fontSize:8,color:"#3a5a70",marginBottom:3 }}>{l}</div><div style={{ fontSize:11,fontWeight:700,color:c,fontFamily:"'Courier New',monospace" }}>{formatCurrency(v)}</div></div>
                      ))}
                    </div>
                    <div style={{ display:"flex",flexDirection:"column",gap:7 }}>
                      {day.installments.sort((a,b)=>a.paid===b.paid?0:a.paid?1:-1).map((inst,idx)=>{
                        ...
                        const isShort=!inst.paid&&inst.paidAmount>0;
                        const sc=inst.paid?(isLate?"#f59e0b":"#4ade80"):isShort?"#f87171":isPast?"#f87171":"#8ab4c8";
                        return (
                          <div key={idx} onClick={()=>{ setSelectedClientId(inst.clientId); setView("detail"); setAcctSelectedDay(null); }} style={{ background:inst.paid?"rgba(34,197,94,0.04)":isShort?"rgba(239,68,68,0.06)":"rgba(255,255,255,0.02)",border:`1px solid ${inst.paid?"rgba(34,197,94,0.15)":isShort?"rgba(239,68,68,0.2)":"rgba(255,255,255,0.04)"}`,borderRadius:10,padding:"10px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer" }}>
                            <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                              <div style={{ width:26,height:26,borderRadius:7,flexShrink:0,display:"flex",alignItems:"center",justifyContent:"center",background:inst.paid?"rgba(34,197,94,0.15)":"rgba(255,255,255,0.05)",color:sc,fontSize:11 }}>{inst.paid?<Icon name="check" size={11}/>:`${inst.day}`}</div>
                              <div>
                                ...
                                {isShort&&<div style={{ fontSize:9,color:"#f87171",marginTop:1 }}>Partial: {formatCurrency(inst.paidAmount)}</div>}
                              </div>
                            </div>
                            <div style={{ textAlign:"right" }}>
                              ...
                              {!inst.paid&&<Badge color={isPast?"#f87171":"#8ab4c8"} bg={isPast?"rgba(239,68,68,0.1)":"rgba(255,255,255,0.05)"}>{isPast?"UNPAID":"DUE"}</Badge>}
                            </div>
                          </div>
                        );
                      })}
    ...
    <div>
      <div style={{ marginBottom:22 }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
          <div>
            ...
            <p style={{ margin:"4px 0 0",color:"#3a5a70",fontSize:13 }}>{isAdmin?`${clients.length} clients · ${pendingLoans.length} pending`:`${visibleClients.length} my clients`}</p>
          </div>
      ...
      {isAdmin&&pendingLoans.length>0&&(
        <div onClick={()=>setView("staff")} style={{ background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.25)",borderRadius:12,padding:"12px 14px",marginBottom:16,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <div style={{ display:"flex",alignItems:"center",gap:8 }}><Icon name="bell" size={16}/><div><div style={{ fontSize:13,fontWeight:700,color:"#f59e0b" }}>{pendingLoans.length} Approval{pendingLoans.length>1?"s":""} Pending</div><div style={{ fontSize:10,color:"#6b5020",marginTop:1 }}>Tap to review</div></div></div>
          <Icon name="chevronDown" size={14}/>
        </div>
      )}

      {!isAdmin&&(()=>{ const mp=pendingLoans.filter(p=>p.requestedBy===currentUser.id); return mp.length>0?(<div style={{ background:"rgba(96,165,250,0.08)",border:"1px solid rgba(96,165,250,0.2)",borderRadius:12,padding:"12px 14px",marginBottom:16 }}><div style={{ fontSize:13,fontWeight:700,color:"#60a5fa",marginBottom:4 }}>⏳ {mp.length} Request{mp.length>1?"s":""} Awaiting Approval</div>{mp.map(p=>{ const cl=clients.find(c=>c.id===p.clientId); return <div key={p.id} style={{ fontSize:11,color:"#3a5a70",marginTop:2 }}>• {cl?.name} — {formatCurrency(p.loanData.principal)}</div>; })}</div>):null; })()}
      ...
      <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:20 }}>
        ...
        <div style={{ gridColumn:"span 2" }}><StatCard label="Overdue Installments" value={`${stats.overdueCount} late`} accent="#ef4444"/></div>
      </div>

      <FinancialPanel fin={globalFin} expandFinancials={expandFinancials} setExpandFinancials={setExpandFinancials}/>
      ...
      {visibleClients.length===0
        ?<div style={{ border:"1px dashed rgba(100,180,255,0.1)",borderRadius:14,padding:40,textAlign:"center",color:"#2a4050" }}><div style={{ fontSize:36,marginBottom:10 }}>💳</div><div style={{ marginBottom:14 }}>No clients yet</div><button onClick={()=>{ setView("clients");setShowAddClient(true); }} style={{ background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",padding:"9px 22px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:13 }}>+ Add First Client</button></div>
        :visibleClients.slice(0,5).map(c=>{ const { active,overdue,paid,total,balance }=getClientLoanSummary(c); return (
          <div key={c.id} onClick={()=>{ setSelectedClientId(c.id);setView("detail"); }} style={{ background:"rgba(255,255,255,0.02)",border:"1px solid rgba(100,180,255,0.07)",borderRadius:12,padding:"12px 15px",cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
            ...
            <div style={{ textAlign:"right" }}>{active?(<><div style={{ fontSize:12,fontWeight:700,color:"#60a5fa",fontFamily:"'Courier New',monospace" }}>{formatCurrency(balance)}</div><div style={{ fontSize:10,color:overdue>0?"#f87171":"#3a6050" }}>{overdue>0?`${overdue} overdue`:`${paid}/${total}`}</div></>):<div style={{ fontSize:11,color:"#2a4050" }}>No loan</div>}</div>
          </div>
        );})}
    </div>
  );
  ...
  const ClientsList = () => (
    <div>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20 }}>
        <div><h1 style={{ margin:0,fontSize:22,fontWeight:800,color:"#e8f4fd" }}>{isAdmin?"All Clients":"My Clients"}</h1></div>
      ...
      <div style={{ position:"relative",marginBottom:10 }}><span style={{ position:"absolute",left:12,top:11,color:"#5a7a90" }}><Icon name="search" size={14}/></span><input type="text" style={{ ...inputStyle,paddingLeft:36 }} placeholder="Search clients…" value={clientSearch} onChange={e=>setClientSearch(e.target.value)}/></div>
      <div style={{ display:"flex",gap:6,marginBottom:16,overflowX:"auto",paddingBottom:4 }}>
        {[{id:"all",l:"All"},{id:"active",l:"Active"},{id:"completed",l:"Completed"},{id:"none",l:"No Loan"}].map(f=>(
      ...
      {filteredClients.length===0
        ...
        :filteredClients.map(c=>{ const { active,overdue,paid,total }=getClientLoanSummary(c); return (
          <div key={c.id} onClick={()=>{ setSelectedClientId(c.id);setView("detail"); }} style={{ background:"rgba(255,255,255,0.025)",border:"1px solid rgba(100,180,255,0.07)",borderRadius:13,padding:"14px 16px",cursor:"pointer",marginBottom:9 }}>
  ...
  const Ledger = () => (
    <div>
      <div style={{ marginBottom:22 }}><h1 style={{ margin:0,fontSize:22,fontWeight:800,color:"#e8f4fd" }}>Ledger Log</h1><p style={{ margin:"4px 0 0",color:"#3a5a70",fontSize:13 }}>All payments in chronological order</p></div>
      {globalTransactions.length===0
        ...
        :globalTransactions.map((tx,idx)=>(
          <div key={idx} style={{ background:"rgba(255,255,255,0.02)",border:`1px solid ${tx.overpayment>0?"rgba(167,139,250,0.15)":tx.shortfall>0?"rgba(239,68,68,0.15)":"rgba(100,180,255,0.05)"}`,borderRadius:12,padding:"12px 14px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
            <div>
              <div style={{ fontWeight:700,color:"#e8f4fd",fontSize:13 }}>{tx.clientName}</div>
              <div style={{ fontSize:10,color:"#3a5a70",marginTop:2 }}>Day {tx.day}/{tx.totalDays} · {formatDate(tx.paidDate)}</div>
    ...
    return (
      <div>
        ...
        <div style={{ background:"rgba(100,180,255,0.04)",border:"1px solid rgba(100,180,255,0.08)",borderRadius:13,padding:"14px 16px",marginBottom:16,display:"grid",gridTemplateColumns:"1fr 1fr",gap:10 }}>
          {[["PHONE",c.phone||"—"],["ID/BVN",c.idNumber||"—"],["SAVINGS",<span style={{ color:"#c084fc",fontWeight:700 }}>{formatCurrency(c.savingsBalance||0)}</span>],["OFFICER",c.assignedToName||"—"]].map(([l,v])=>(
            <div key={l}><div style={{ fontSize:10,color:"#3a5a70",marginBottom:3 }}>{l}</div><div style={{ color:"#c8dde8",fontSize:13 }}>{v}</div></div>
          ))}
        ...
        {myPendingForClient.length>0&&<div style={{ background:"rgba(96,165,250,0.08)",border:"1px solid rgba(96,165,250,0.2)",borderRadius:12,padding:"12px 14px",marginBottom:14 }}><div style={{ fontSize:13,fontWeight:700,color:"#60a5fa",marginBottom:4 }}>⏳ Loan Request Pending Approval</div>{myPendingForClient.map(p=><div key={p.id} style={{ fontSize:11,color:"#3a5a70" }}>{formatCurrency(p.loanData.principal)} · {p.loanData.days}d · {formatDate(p.requestedAt?.split("T")[0])}</div>)}</div>}
        ...
        {activeTab==="loans"&&(
          allLoans.length===0
            ?<div style={{ border:"1px dashed rgba(100,180,255,0.08)",borderRadius:13,padding:40,textAlign:"center",color:"#2a4050" }}><div style={{ fontSize:32,marginBottom:10 }}>💰</div><div>No loan cycles yet.</div></div>
            :<div>
            ...
            {(!c.savingsLogs||c.savingsLogs.length===0)
              ...
              :c.savingsLogs.map((log,idx)=>(
                <div key={idx} style={{ background:"rgba(255,255,255,0.01)",border:"1px solid rgba(168,85,247,0.08)",borderRadius:10,padding:"10px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:6 }}>
                  <div><div style={{ fontSize:12,fontWeight:600,color:log.type==="deposit"?"#4ade80":log.type==="withdraw"?"#f87171":"#c084fc" }}>{log.type==="deposit"?"Deposit":log.type==="withdraw"?"Withdrawal":"Admin Override"}</div><div style={{ fontSize:9,color:"#3a5a70",marginTop:2 }}>{formatDate(log.date)}{log.recordedBy&&` · ${log.recordedBy}`}</div></div>
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#060f1a",fontFamily:"'Segoe UI',system-ui,sans-serif",color:"#e8f4fd" }}>
      ...
      {/* Officer Banner */}
      {!isAdmin&&<div style={{ background:"rgba(96,165,250,0.12)",borderBottom:"1px solid rgba(96,165,250,0.15)",padding:"5px 16px",fontSize:11,color:"#60a5fa",display:"flex",justifyContent:"space-between" }}><span>🏦 Loan Officer — Only your assigned clients</span><span style={{ color:"#3a5a70" }}>{currentUser.name}</span></div>}
      ...
      <div style={{ background:"rgba(6,15,26,0.95)",borderBottom:"1px solid rgba(100,180,255,0.07)",padding:"14px 18px",display:"flex",justifyContent:"space-between",alignItems:"center",position:"sticky",top:!isAdmin?28:0,zIndex:100,backdropFilter:"blur(10px)" }}>
        ...
        <div style={{ display:"flex",alignItems:"center",gap:10 }}>
          <span style={{ fontSize:10,color:sync==="synced"?"#22c55e":"#60a5fa",fontWeight:600 }}>{sync==="synced"?"☁️ Synced":"⟳ Syncing"}</span>
      ...
      {showProfile&&(
        <Modal title="👤 My Profile" onClose={()=>setShowProfile(false)}>
          ...
          <div style={{ background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.2)",borderRadius:9,padding:"9px 12px",marginBottom:16,fontSize:12,color:"#f59e0b" }}>🔐 Enter current password to save any changes.</div>
          <Field label="Current Password *"><input type="password" style={inputStyle} value={profileEdit.oldPassword} onChange={e=>setProfileEdit(p=>({...p,oldPassword:e.target.value}))} placeholder="Your current password"/></Field>
          ...
          <Field label="New Username"><input style={inputStyle} value={profileEdit.username} onChange={e=>setProfileEdit(p=>({...p,username:e.target.value.toLowerCase().replace(/\s/g,"")}))} placeholder="Leave blank to keep current"/></Field>
          <Field label="New Password"><input type="password" style={inputStyle} value={profileEdit.newPassword} onChange={e=>setProfileEdit(p=>({...p,newPassword:e.target.value}))} placeholder="Leave blank to keep current"/></Field>
          ...
          <button onClick={handleUpdateProfile} style={{ width:"100%",background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",padding:"12px",borderRadius:12,cursor:"pointer",fontWeight:800,fontSize:14 }}>✓ Update Profile</button>
        </Modal>
      )}
        ...
        return(
          <Modal title="Record Payment" onClose={()=>{setShowPayment(null);setPaymentAmount("");setPaymentPreview(null);}}>
            <div style={{ textAlign:"center",marginBottom:16 }}>
              ...
              <div style={{ fontSize:11,color:"#3a5a70",marginTop:4 }}>Day {showPayment.scheduleIdx+1} of {ln?.days} · {cl?.name}</div>
            </div>
            ...
            {preview&&amount>0&&(
              <div style={{ marginBottom:14 }}>
                {isOver&&<div style={{ background:"rgba(167,139,250,0.1)",border:"1px solid rgba(167,139,250,0.25)",borderRadius:12,padding:"12px 14px" }}>
            ...
            <button onClick={handlePayment} style={{ width:"100%",background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",padding:"12px",borderRadius:12,cursor:"pointer",fontWeight:800,fontSize:14,marginTop:6 }}>
              ✓ Confirm {isOver?"(+Cascade)":isUnder?"(Partial)":""}
            </button>
          </Modal>
        );
      })()}
      ...
      {adminEditInstallment&&(
        <Modal title="🛠️ Admin Override" onClose={()=>setAdminEditInstallment(null)}>
      ...
      {showAddClient&&(
        <Modal title="Register New Client" onClose={()=>setShowAddClient(false)}>
          <Field label="Full Name *"><input style={inputStyle} value={newClient.name} onChange={e=>setNewClient(p=>({...p,name:e.target.value}))} placeholder="e.g. Amaka Johnson" autoFocus/></Field>
          ...
          <Field label="ID / BVN"><input style={inputStyle} value={newClient.idNumber} onChange={e=>setNewClient(p=>({...p,idNumber:e.target.value}))} placeholder="National ID or BVN"/></Field>
      ...
      {/* Add / Request Loan */}
      {showAddLoan&&(
        <Modal title={isAdmin?"Issue New Loan":"Request Loan Approval"} onClose={()=>setShowAddLoan(false)}>
          <div style={{ background:isAdmin?"rgba(59,130,246,0.08)":"rgba(245,158,11,0.08)",border:`1px solid ${isAdmin?"rgba(59,130,246,0.15)":"rgba(245,158,11,0.2)"}`,borderRadius:9,padding:"9px 13px",marginBottom:16,fontSize:13,color:isAdmin?"#93c5fd":"#f59e0b" }}>
            {isAdmin?"Issuing for: ":"Requesting for: "}<strong>{selectedClient?.name}</strong>{!isAdmin&&<span style={{ fontSize:11,color:"#6b5020",display:"block",marginTop:3 }}>Admin will review and approve.</span>}
          </div>
          <Field label="Principal (₦) *"><input type="number" style={inputStyle} value={newLoan.principal} onChange={e=>setNewLoan(p=>({...p,principal:e.target.value}))} placeholder="e.g. 50000"/></Field>
          ...
          <Field label="Duration (Days)"><input type="number" style={inputStyle} value={newLoan.days} onChange={e=>setNewLoan(p=>({...p,days:e.target.value}))} placeholder="e.g. 20"/></Field>
          ...
          <button onClick={isAdmin?handleAddLoan:handleRequestLoan} style={{ width:"100%",background:isAdmin?"linear-gradient(135deg,#22c55e,#16a34a)":"linear-gradient(135deg,#f59e0b,#d97706)",border:"none",color:"#000",padding:"12px",borderRadius:12,cursor:"pointer",fontWeight:800,fontSize:14 }}>
            {isAdmin?`✓ Issue Cycle #${(selectedClient?.loans?.length||0)+1}`:"📤 Submit for Approval"}
          </button>
        </Modal>
      )}
      ...
      {showSettings&&(
        <Modal title="⚙️ Settings & Backup" onClose={()=>setShowSettings(false)}>
          <div style={{ display:"flex",flexDirection:"column",gap:14 }}>
            ...
            <div style={{ background:"rgba(34,197,94,0.06)",border:"1px solid rgba(34,197,94,0.15)",borderRadius:10,padding:"10px 14px" }}><div style={{ fontSize:12,color:"#4ade80",fontWeight:700 }}>☁️ Firebase Cloud Sync</div><div style={{ fontSize:11,color:"#3a6050",marginTop:3 }}>Data syncs in real-time across all devices. Project: creda-finance</div></div>
            {isAdmin&&<>
              <div><div style={{ fontSize:12,color:"#8a9bb0",marginBottom:8,fontWeight:600 }}>📤 EXPORT</div><button onClick={()=>{exportData();setShowSettings(false);}} style={{ width:"100%",background:"linear-gradient(135deg,#22c55e,#16a34a)",border:"none",color:"#000",padding:"11px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:13 }}>Download Backup (.json)</button></div>
              <div><div style={{ fontSize:12,color:"#8a9bb0",marginBottom:8,fontWeight:600 }}>📥 RESTORE</div>
                <label style={{ display:"block",width:"100%",background:"rgba(96,165,250,0.1)",border:"1px solid rgba(96,165,250,0.25)",color:"#60a5fa",padding:"11px",borderRadius:10,cursor:"pointer",fontWeight:700,fontSize:13,textAlign:"center",boxSizing:"border-box" }}>Choose Backup File
                  <input type="file" accept=".json" style={{ display:"none" }} onChange={e=>{ if(e.target.files[0]){ const reader=new FileReader(); reader.onload=ev=>{ try{ const data=JSON.parse(ev.target.result); if(data.clients&&Array.isArray(data.clients)){ saveClients(data.clients); if(data.users)saveUsers(data.users); if(data.pendingLoans)savePending(data.pendingLoans); setShowSettings(false); showToast(`✅ ${data.clients.length} clients restored!`); }else showToast("Invalid file","error"); }catch{ showToast("Could not read file","error"); } }; reader.readAsText(e.target.files[0]); }}}/>
                </label>
              </div>
              <div style={{ borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:14 }}>
                <div style={{ background:"rgba(255,255,255,0.03)",borderRadius:8,padding:"10px 12px",display:"flex",flexDirection:"column",gap:6 }}>
                  {[["Total Clients",clients.length,"#fff"],["Officers",users.filter(u=>u.role==="loan_officer").length,"#60a5fa"],["Active Loans",clients.reduce((a,c)=>a+(c.loans?.filter(l=>l.status==="active").length||0),0),"#4ade80"],["Pending",pendingLoans.length,"#f59e0b"]].map(([l,v,c])=>(
              ...
              <button onClick={()=>{ if(window.confirm("Delete ALL client data from Firebase?")){ saveClients([]); savePending([]); setShowSettings(false); showToast("Data cleared","error"); }}} style={{ width:"100%",background:"rgba(248,113,113,0.1)",border:"1px solid rgba(248,113,113,0.2)",color:"#f87171",padding:"10px",borderRadius:10,cursor:"pointer",fontWeight:600,fontSize:12 }}>🗑️ Clear All Client Data</button>
            </>}
      ...
      {confirmDelete&&(
        <Modal title="Delete Client?" onClose={()=>setConfirmDelete(null)}>
          <p style={{ color:"#8ab4c8",fontSize:14,marginBottom:20,lineHeight:1.6 }}>This will permanently delete this client and all their records from Firebase.</p>
          <div style={{ display:"flex",gap:10 }}>
            <button onClick={()=>setConfirmDelete(null)} style={{ flex:1,background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",color:"#8ab4c8",padding:"11px",borderRadius:11,cursor:"pointer",fontWeight:700 }}>Cancel</button>
            <button onClick={()=>handleDeleteClient(confirmDelete)} style={{ flex:1,background:"linear-gradient(135deg,#dc2626,#b91c1c)",border:"none",color:"#fff",padding:"11px",borderRadius:11,cursor:"pointer",fontWeight:700 }}>Delete</button>
          </div>
        </Modal>
      )}
    </div>
