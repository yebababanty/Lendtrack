...
// ─── FIREBASE HELPERS ─────────────────────────────────────────────────────────
const CLIENTS_COL = "creda_clients";
const USERS_COL = "creda_users";
const PENDING_COL = "creda_pending_loans";

async function saveClientsToFirebase(clients) {
  try {
    const batch = writeBatch(db);
    clients.forEach(c => {
      const ref = doc(db, CLIENTS_COL, c.id);
      batch.set(ref, c);
    });
    await batch.commit();
  } catch (e) { console.error("Firebase save clients failed:", e); }
}
...
async function saveUsersToFirebase(users) {
  try {
    ...
    await batch.commit();
  } catch (e) { console.error("Firebase save users failed:", e); }
}

async function savePendingToFirebase(pendingLoans) {
  try {
    const batch = writeBatch(db);
    // Clear old ones then write new
    pendingLoans.forEach(p => {
      const ref = doc(db, PENDING_COL, p.id);
      batch.set(ref, p);
    });
    await batch.commit();
  } catch (e) { console.error("Firebase save pending failed:", e); }
}
...
function computeCollectiveReport(clients) {
  const totalClients = clients.length;
  const totalLoans = clients.reduce((a, c) => a + (c.loans?.length || 0), 0);
  const activeLoans = clients.reduce((a, c) => a + (c.loans?.filter(l => l.status === "active").length || 0), 0);
  const completedLoans = clients.reduce((a, c) => a + (c.loans?.filter(l => l.status === "completed").length || 0), 0);
  let totalDisbursed = 0, totalRepayable = 0, totalCollected = 0, totalInterest = 0;
  let onTimeCount = 0, lateCount = 0, overdueCount = 0, partialCount = 0, totalInstallments = 0;
  clients.forEach(c => {
    (c.loans || []).forEach(l => {
      ...
      (l.schedule || []).forEach(s => {
        ...
        if (s.paid && s.paidDate <= s.dueDate) onTimeCount++;
        if (s.paid && s.paidDate > s.dueDate) lateCount++;
        if (!s.paid && new Date(s.dueDate) < new Date()) overdueCount++;
        if (!s.paid && s.paidAmount > 0) partialCount++;
      });
    });
  });
  const collectionRate = totalRepayable > 0 ? (totalCollected / totalRepayable) * 100 : 0;
  const onTimeRate = totalInstallments > 0 ? (onTimeCount / totalInstallments) * 100 : 0;
  const defaultRate = totalLoans > 0 ? (overdueCount / totalInstallments) * 100 : 0;
  const avgLoanSize = totalLoans > 0 ? totalDisbursed / totalLoans : 0;
  return {
    totalClients, totalLoans, activeLoans, completedLoans,
    totalDisbursed, totalRepayable, totalCollected, totalInterest,
    outstanding: Math.max(0, totalRepayable - totalCollected),
    collectionRate, onTimeRate, defaultRate, avgLoanSize,
    onTimeCount, lateCount, overdueCount, partialCount, totalInstallments,
    trueProfit: totalCollected > 0 ? totalCollected - (totalCollected * (totalDisbursed / totalRepayable)) : 0,
    realROI: totalDisbursed > 0 ? ((totalCollected - totalDisbursed) / totalDisbursed) * 100 : 0,
  };
}

function computeIndividualReport(client) {
  ...
  const creditScore = Math.min(100, Math.round(
    (onTimeRate * 0.5) + (collectionRate * 0.3) + (completedLoans / Math.max(totalLoans, 1) * 20)
  ));
  const creditGrade = creditScore >= 85 ? "A+" : creditScore >= 75 ? "A" : creditScore >= 65 ? "B" : creditScore >= 50 ? "C" : "D";
  const creditColor = creditScore >= 75 ? "#22c55e" : creditScore >= 50 ? "#f59e0b" : "#ef4444";
  return {
    totalLoans, activeLoans, completedLoans,
    totalDisbursed, totalRepayable, totalCollected, totalInterest,
    outstanding: Math.max(0, totalRepayable - totalCollected),
    collectionRate, onTimeRate, creditScore, creditGrade, creditColor,
    onTimeCount, lateCount, overdueCount, partialCount, totalInstallments,
    realROI: totalDisbursed > 0 ? ((totalCollected - totalDisbursed) / totalDisbursed) * 100 : 0,
  };
}
...
function exportReportAsText(title, lines) {
  const content = [title, "=".repeat(50), ...lines, "", `Generated: ${new Date().toLocaleString()}`].join("\n");
...
function ProgressBar({ value, max = 100, color = "#22c55e", height = 5 }) {
  const pct = max > 0 ? Math.min((value / max) * 100, 100) : 0;
  return (
    <div style={{ height,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden" }}>
...
function ReportsView({ clients, users, isAdmin }) {
  const [reportTab, setReportTab] = useState("collective");
  const [selectedClientId, setSelectedClientId] = useState(null);
  const [clientSearch, setClientSearch] = useState("");

  const collectiveReport = useMemo(() => computeCollectiveReport(clients), [clients]);
  ...
  const individualReport = useMemo(() =>
    selectedClient ? computeIndividualReport(selectedClient) : null,
    [selectedClient]
  );

  const handleExportCollective = () => {
    const r = collectiveReport;
    exportReportAsText("CREDA COLLECTIVE PERFORMANCE REPORT", [
      `Date: ${new Date().toLocaleDateString()}`,
      "",
      "── PORTFOLIO OVERVIEW ──",
      `Total Clients:        ${r.totalClients}`,
      `Total Loan Cycles:    ${r.totalLoans}`,
      `Active Loans:         ${r.activeLoans}`,
      `Completed Loans:      ${r.completedLoans}`,
      "",
      "── FINANCIALS ──",
      `Total Disbursed:      ${formatCurrency(r.totalDisbursed)}`,
      `Total Repayable:      ${formatCurrency(r.totalRepayable)}`,
      `Total Collected:      ${formatCurrency(r.totalCollected)}`,
      `Outstanding:          ${formatCurrency(r.outstanding)}`,
      `Total Interest:       ${formatCurrency(r.totalInterest)}`,
      `Average Loan Size:    ${formatCurrency(r.avgLoanSize)}`,
      "",
      "── PERFORMANCE ──",
      `Collection Rate:      ${r.collectionRate.toFixed(2)}%`,
      `On-Time Rate:         ${r.onTimeRate.toFixed(2)}%`,
      `Default Rate:         ${r.defaultRate.toFixed(2)}%`,
      `Real ROI:             ${r.realROI.toFixed(2)}%`,
      "",
  ...
  const handleExportIndividual = () => {
    if (!selectedClient || !individualReport) return;
    const r = individualReport;
    const c = selectedClient;
    exportReportAsText(`CREDA INDIVIDUAL REPORT — ${c.name.toUpperCase()}`, [
      `Client ID:            ${c.id}`,
      `Phone:                ${c.phone}`,
      `Address:              ${c.address || "—"}`,
      `ID/BVN:               ${c.idNumber || "—"}`,
      `Loan Officer:         ${c.assignedToName || "—"}`,
      `Date:                 ${new Date().toLocaleDateString()}`,
      "",
      "── CREDIT SCORE ──",
      `Credit Score:         ${r.creditScore}/100`,
      `Credit Grade:         ${r.creditGrade}`,
      "",
      "── LOAN HISTORY ──",
      `Total Cycles:         ${r.totalLoans}`,
      `Active:               ${r.activeLoans}`,
      `Completed:            ${r.completedLoans}`,
      "",
      "── FINANCIALS ──",
      `Total Borrowed:       ${formatCurrency(r.totalDisbursed)}`,
      `Total Repayable:      ${formatCurrency(r.totalRepayable)}`,
      `Total Collected:      ${formatCurrency(r.totalCollected)}`,
      `Outstanding:          ${formatCurrency(r.outstanding)}`,
      `Total Interest:       ${formatCurrency(r.totalInterest)}`,
      `Savings Balance:      ${formatCurrency(c.savingsBalance || 0)}`,
      "",
      "── PERFORMANCE ──",
      `Collection Rate:      ${r.collectionRate.toFixed(2)}%`,
      `On-Time Rate:         ${r.onTimeRate.toFixed(2)}%`,
  ...
  // Rank clients by credit score
  const clientRankings = useMemo(() => {
    return clients.map(c => {
      const r = computeIndividualReport(c);
      return { ...c, report: r };
    }).sort((a, b) => b.report.creditScore - a.report.creditScore);
  }, [clients]);
  ...
  return (
    <div>
      <div style={{ marginBottom:22 }}>
        ...
        <p style={{ margin:"4px 0 0",color:"#3a5a70",fontSize:13 }}>Collective portfolio & individual client analytics</p>
      </div>

      {/* Tab Switch */}
      <div style={{ display:"flex",gap:3,marginBottom:20,background:"rgba(255,255,255,0.03)",borderRadius:12,padding:4 }}>
        {[
          {id:"collective",label:"📊 Portfolio"},
          {id:"individual",label:"👤 Individual"},
          {id:"rankings",label:"🏆 Rankings"},
        ].map(tab=>(
          <button key={tab.id} onClick={()=>setReportTab(tab.id)} style={{ flex:1,padding:"9px 6px",borderRadius:9,border:"none",cursor:"pointer",background:reportTab===tab.id?"rgba(100,180,255,0.12)":"transparent",color:reportTab===tab.id?"#93c5fd":"#3a5a70",fontWeight:700,fontSize:11,transition:"all 0.2s" }}>{tab.label}</button>
        ))}
      </div>

      {/* ── COLLECTIVE REPORT ── */}
      {reportTab === "collective" && (
        <div>
          {/* Export Button */}
          <button onClick={handleExportCollective} style={{ width:"100%",background:"linear-gradient(135deg,#3b82f6,#2563eb)",border:"none",color:"#fff",padding:"11px",borderRadius:12,cursor:"pointer",fontWeight:700,fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",gap:8,marginBottom:18 }}>
            <Icon name="download" size={14}/>Download Portfolio Report (.txt)
          </button>

          {/* Portfolio Summary */}
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:18 }}>
            {[
              ["Total Clients",collectiveReport.totalClients,"#60a5fa","#3b82f6"],
              ["Loan Cycles",collectiveReport.totalLoans,"#4ade80","#22c55e"],
              ["Active Loans",collectiveReport.activeLoans,"#f59e0b","#f59e0b"],
              ["Completed",collectiveReport.completedLoans,"#a78bfa","#a855f7"],
            ].map(([l,v,c,accent])=>(
              <div key={l} style={{ background:"rgba(255,255,255,0.03)",borderRadius:14,padding:"14px 16px",borderTop:`2px solid ${accent}` }}>
                <div style={{ fontSize:10,color:"#4a6880",letterSpacing:1,textTransform:"uppercase",marginBottom:6 }}>{l}</div>
          ...
          {/* Financial Overview */}
          <div style={{ background:"rgba(255,255,255,0.02)",border:"1px solid rgba(100,180,255,0.08)",borderRadius:14,padding:"14px 16px",marginBottom:16 }}>
            <div style={{ fontSize:11,fontWeight:700,color:"#5a7a90",letterSpacing:1,textTransform:"uppercase",marginBottom:12 }}>Financial Overview</div>
            <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
              {[
                ["Total Disbursed",formatCurrency(collectiveReport.totalDisbursed),"#60a5fa"],
                ["Total Repayable",formatCurrency(collectiveReport.totalRepayable),"#c4b5fd"],
                ["Total Collected",formatCurrency(collectiveReport.totalCollected),"#4ade80"],
                ["Outstanding",formatCurrency(collectiveReport.outstanding),"#f87171"],
                ["Total Interest",formatCurrency(collectiveReport.totalInterest),"#a78bfa"],
                ["Avg Loan Size",formatCurrency(collectiveReport.avgLoanSize),"#f59e0b"],
              ].map(([l,v,c])=>(
                <div key={l} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:"rgba(0,0,0,0.15)",borderRadius:9 }}>
                  <span style={{ fontSize:12,color:"#8ab4c8" }}>{l}</span>
          ...
          {/* Performance Metrics */}
          <div style={{ background:"rgba(255,255,255,0.02)",border:"1px solid rgba(100,180,255,0.08)",borderRadius:14,padding:"14px 16px",marginBottom:16 }}>
            <div style={{ fontSize:11,fontWeight:700,color:"#5a7a90",letterSpacing:1,textTransform:"uppercase",marginBottom:12 }}>Performance Metrics</div>
            <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
              {[
                ["Collection Rate",collectiveReport.collectionRate,"#22c55e"],
                ["On-Time Payment Rate",collectiveReport.onTimeRate,"#60a5fa"],
                ["Real ROI",Math.max(0,collectiveReport.realROI),"#a78bfa"],
              ].map(([l,v,c])=>(
                <div key={l}>
                  <div style={{ display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:5 }}>
                    <span style={{ color:"#8ab4c8" }}>{l}</span>
                    <span style={{ fontWeight:700,color:c,fontFamily:"'Courier New',monospace" }}>{v.toFixed(1)}%</span>
                  </div>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:16 }}>
            {[
              ["On Time",collectiveReport.onTimeCount,"#4ade80","rgba(34,197,94,0.08)","rgba(34,197,94,0.2)"],
              ["Late",collectiveReport.lateCount,"#f59e0b","rgba(245,158,11,0.08)","rgba(245,158,11,0.2)"],
              ["Overdue",collectiveReport.overdueCount,"#f87171","rgba(239,68,68,0.08)","rgba(239,68,68,0.2)"],
              ["Partial",collectiveReport.partialCount,"#a78bfa","rgba(167,139,250,0.08)","rgba(167,139,250,0.2)"],
            ].map(([l,v,c,bg,border])=>(
              <div key={l} style={{ background:bg,border:`1px solid ${border}`,borderRadius:12,padding:"12px 14px",textAlign:"center" }}>
                <div style={{ fontSize:24,fontWeight:900,color:c,marginBottom:4 }}>{v}</div>
                <div style={{ fontSize:10,color:"#5a7a90",textTransform:"uppercase",letterSpacing:0.8 }}>{l}</div>
                <div style={{ marginTop:6 }}>
                  <ProgressBar value={v} max={collectiveReport.totalInstallments} color={c} height={3}/>
                </div>
                <div style={{ fontSize:9,color:"#3a5a70",marginTop:3 }}>
                  {collectiveReport.totalInstallments > 0 ? ((v/collectiveReport.totalInstallments)*100).toFixed(1) : 0}% of total
                </div>
              </div>
            ))}
          </div>
          ...
          {/* Default Rate Alert */}
          {collectiveReport.defaultRate > 10 && (
            <div style={{ background:"rgba(239,68,68,0.08)",border:"1px solid rgba(239,68,68,0.2)",borderRadius:12,padding:"12px 14px",marginBottom:16 }}>
              <div style={{ fontSize:13,fontWeight:700,color:"#f87171",marginBottom:4 }}>⚠ High Default Rate Detected</div>
              <div style={{ fontSize:11,color:"#8a3030" }}>Default rate is at {collectiveReport.defaultRate.toFixed(1)}%. Consider reviewing collection processes.</div>
            </div>
          )}
        </div>
      )}
          ...
          {!selectedClient ? (
            <div>
              <div style={{ position:"relative",marginBottom:14 }}>
                ...
                <input type="text" style={{ ...inputStyle,paddingLeft:36 }} placeholder="Search client..." value={clientSearch} onChange={e=>setClientSearch(e.target.value)}/>
              </div>
              <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
                {filteredClients.map(c => {
                  const r = computeIndividualReport(c);
                  return (
                    <div key={c.id} onClick={()=>setSelectedClientId(c.id)} style={{ background:"rgba(255,255,255,0.025)",border:"1px solid rgba(100,180,255,0.08)",borderRadius:13,padding:"13px 15px",cursor:"pointer" }}>
                      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
                        <div style={{ display:"flex",alignItems:"center",gap:9 }}>
                          ...
                          <div>
                            <div style={{ fontWeight:700,color:"#dceef8",fontSize:13 }}>{c.name}</div>
                            <div style={{ fontSize:10,color:"#3a5a70",marginTop:1 }}>{r.totalLoans} cycles · {c.assignedToName||"—"}</div>
                          </div>
                        </div>
                        <div style={{ textAlign:"right" }}>
                          <div style={{ fontSize:18,fontWeight:900,color:r.creditColor,fontFamily:"'Courier New',monospace" }}>{r.creditGrade}</div>
                          <div style={{ fontSize:9,color:"#3a5a70" }}>{r.creditScore}/100</div>
                        </div>
                      </div>
                      <div style={{ display:"flex",gap:8 }}>
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:9,color:"#3a5a70",marginBottom:3 }}>Collection Rate</div>
                          <ProgressBar value={r.collectionRate} color={r.creditColor} height={4}/>
                          <div style={{ fontSize:9,color:r.creditColor,marginTop:2,fontWeight:600 }}>{r.collectionRate.toFixed(1)}%</div>
                        </div>
                        <div style={{ flex:1 }}>
                          <div style={{ fontSize:9,color:"#3a5a70",marginBottom:3 }}>On-Time Rate</div>
                          <ProgressBar value={r.onTimeRate} color="#60a5fa" height={4}/>
                          <div style={{ fontSize:9,color:"#60a5fa",marginTop:2,fontWeight:600 }}>{r.onTimeRate.toFixed(1)}%</div>
                        </div>
                      </div>
                    </div>
                  );
            ...
            <div>
              {/* Back */}
              <button onClick={()=>setSelectedClientId(null)} style={{ display:"flex",alignItems:"center",gap:6,background:"rgba(255,255,255,0.05)",border:"none",color:"#8ab4c8",padding:"8px 14px",borderRadius:9,cursor:"pointer",marginBottom:16,fontWeight:600,fontSize:12 }}>
                <Icon name="back" size={13}/>Back to list
              </button>

              {/* Client Header */}
              <div style={{ background:"rgba(255,255,255,0.03)",border:"1px solid rgba(100,180,255,0.1)",borderRadius:14,padding:"14px 16px",marginBottom:16 }}>
                <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                  <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                    <div style={{ width:44,height:44,borderRadius:12,background:"linear-gradient(135deg,#1d4ed8,#3b82f6)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:18,color:"#fff" }}>{selectedClient.name.charAt(0).toUpperCase()}</div>
                    <div>
                      <div style={{ fontSize:16,fontWeight:800,color:"#e8f4fd" }}>{selectedClient.name}</div>
                      <div style={{ fontSize:11,color:"#3a5a70" }}>{selectedClient.id} · {selectedClient.phone}</div>
                    </div>
                  </div>
                  {/* Credit Score Badge */}
                  <div style={{ textAlign:"center",background:`${individualReport.creditColor}15`,border:`1px solid ${individualReport.creditColor}40`,borderRadius:12,padding:"8px 14px" }}>
                    <div style={{ fontSize:24,fontWeight:900,color:individualReport.creditColor,fontFamily:"'Courier New',monospace" }}>{individualReport.creditGrade}</div>
                    <div style={{ fontSize:9,color:"#3a5a70",marginTop:2 }}>CREDIT GRADE</div>
                    <div style={{ fontSize:11,fontWeight:700,color:individualReport.creditColor }}>{individualReport.creditScore}/100</div>
                  </div>
                </div>
              </div>

              {/* Export */}
              <button onClick={handleExportIndividual} style={{ width:"100%",background:"linear-gradient(135deg,#3b82f6,#2563eb)",border:"none",color:"#fff",padding:"10px",borderRadius:11,cursor:"pointer",fontWeight:700,fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",gap:7,marginBottom:16 }}>
                <Icon name="download" size={13}/>Download Individual Report (.txt)
              </button>

              {/* Credit Score Bar */}
              <div style={{ background:"rgba(255,255,255,0.02)",border:"1px solid rgba(100,180,255,0.08)",borderRadius:14,padding:"14px 16px",marginBottom:16 }}>
                <div style={{ fontSize:11,fontWeight:700,color:"#5a7a90",letterSpacing:1,textTransform:"uppercase",marginBottom:10 }}>Credit Score Breakdown</div>
                <div style={{ marginBottom:8 }}>
                  <div style={{ display:"flex",justifyContent:"space-between",fontSize:10,color:"#3a5a70",marginBottom:5 }}>
                    <span>Credit Score</span>
                    <span style={{ color:individualReport.creditColor,fontWeight:700 }}>{individualReport.creditScore}/100</span>
                  </div>
                  <ProgressBar value={individualReport.creditScore} color={individualReport.creditColor} height={8}/>
                </div>
                <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr",gap:6,marginTop:10 }}>
                  {[
                    ["On-Time Rate",individualReport.onTimeRate.toFixed(1)+"%","50% weight","#60a5fa"],
                    ["Collection Rate",individualReport.collectionRate.toFixed(1)+"%","30% weight","#4ade80"],
                    ["Completion Ratio",(individualReport.completedLoans/Math.max(individualReport.totalLoans,1)*100).toFixed(0)+"%","20% weight","#a78bfa"],
                    ["Grade",individualReport.creditGrade,"Final","#f59e0b"],
                  ].map(([l,v,sub,c])=>(
                    <div key={l} style={{ background:"rgba(0,0,0,0.2)",borderRadius:9,padding:"9px 11px" }}>
                      ...
                      <div style={{ fontSize:8,color:"#2a3a48",marginTop:1 }}>{sub}</div>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div style={{ background:"rgba(255,255,255,0.02)",border:"1px solid rgba(100,180,255,0.08)",borderRadius:14,padding:"14px 16px",marginBottom:16 }}>
                <div style={{ fontSize:11,fontWeight:700,color:"#5a7a90",letterSpacing:1,textTransform:"uppercase",marginBottom:12 }}>Financial Summary</div>
                <div style={{ display:"flex",flexDirection:"column",gap:7 }}>
                  {[
                    ["Total Borrowed",formatCurrency(individualReport.totalDisbursed),"#60a5fa"],
                    ["Total Repayable",formatCurrency(individualReport.totalRepayable),"#c4b5fd"],
                    ["Total Paid",formatCurrency(individualReport.totalCollected),"#4ade80"],
                    ["Outstanding",formatCurrency(individualReport.outstanding),individualReport.outstanding>0?"#f87171":"#4ade80"],
                    ["Total Interest",formatCurrency(individualReport.totalInterest),"#a78bfa"],
                    ["Savings Balance",formatCurrency(selectedClient.savingsBalance||0),"#c084fc"],
                  ].map(([l,v,c])=>(
                    <div key={l} style={{ display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 12px",background:"rgba(0,0,0,0.15)",borderRadius:9 }}>
                      <span style={{ fontSize:12,color:"#8ab4c8" }}>{l}</span>
                      <span style={{ fontSize:13,fontWeight:700,color:c,fontFamily:"'Courier New',monospace" }}>{v}</span>
              ...
              {/* Performance */}
              <div style={{ background:"rgba(255,255,255,0.02)",border:"1px solid rgba(100,180,255,0.08)",borderRadius:14,padding:"14px 16px",marginBottom:16 }}>
                <div style={{ fontSize:11,fontWeight:700,color:"#5a7a90",letterSpacing:1,textTransform:"uppercase",marginBottom:12 }}>Payment Performance</div>
                <div style={{ display:"flex",flexDirection:"column",gap:12 }}>
                  {[
                    ["Collection Rate",individualReport.collectionRate,"#22c55e"],
                    ["On-Time Rate",individualReport.onTimeRate,"#60a5fa"],
                  ].map(([l,v,c])=>(
                    <div key={l}>
                      <div style={{ display:"flex",justifyContent:"space-between",fontSize:11,marginBottom:5 }}>
                        <span style={{ color:"#8ab4c8" }}>{l}</span>
                ...
                <div style={{ display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:6,marginTop:14 }}>
                  {[
                    ["On Time",individualReport.onTimeCount,"#4ade80","rgba(34,197,94,0.08)"],
                    ["Late",individualReport.lateCount,"#f59e0b","rgba(245,158,11,0.08)"],
                    ["Overdue",individualReport.overdueCount,"#f87171","rgba(239,68,68,0.08)"],
                    ["Partial",individualReport.partialCount,"#a78bfa","rgba(167,139,250,0.08)"],
                  ].map(([l,v,c,bg])=>(
      ...
      {reportTab === "rankings" && (
        <div>
          <div style={{ background:"rgba(245,158,11,0.06)",border:"1px solid rgba(245,158,11,0.15)",borderRadius:12,padding:"11px 14px",marginBottom:16,fontSize:12,color:"#f59e0b" }}>
            🏆 Clients ranked by credit score (on-time payments + collection rate + completions)
          </div>
          <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
            {clientRankings.map((c, idx) => {
              const r = c.report;
              const medal = idx === 0 ? "🥇" : idx === 1 ? "🥈" : idx === 2 ? "🥉" : `#${idx+1}`;
              return (
                <div key={c.id} style={{ background:"rgba(255,255,255,0.025)",border:`1px solid ${r.creditColor}25`,borderRadius:13,padding:"12px 14px" }}>
                  <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8 }}>
                    <div style={{ display:"flex",alignItems:"center",gap:10 }}>
                      <div style={{ fontSize:idx<3?22:14,fontWeight:800,minWidth:32,textAlign:"center" }}>{medal}</div>
                      <div style={{ width:36,height:36,borderRadius:10,background:"linear-gradient(135deg,#1d4ed8,#3b82f6)",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:800,fontSize:15,color:"#fff",flexShrink:0 }}>{c.name.charAt(0).toUpperCase()}</div>
                      <div>
                        <div style={{ fontWeight:700,color:"#dceef8",fontSize:13 }}>{c.name}</div>
                        <div style={{ fontSize:10,color:"#3a5a70" }}>{r.totalLoans} cycles · {formatCurrency(r.totalDisbursed)}</div>
                      </div>
                    </div>
                    <div style={{ textAlign:"right" }}>
                      <div style={{ fontSize:20,fontWeight:900,color:r.creditColor,fontFamily:"'Courier New',monospace" }}>{r.creditGrade}</div>
                      <div style={{ fontSize:10,color:"#3a5a70" }}>{r.creditScore}/100</div>
                    </div>
                  </div>
                  <div style={{ display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:6 }}>
                    {[
                      ["Collection",r.collectionRate.toFixed(0)+"%","#4ade80"],
                      ["On Time",r.onTimeRate.toFixed(0)+"%","#60a5fa"],
                      ["Overdue",r.overdueCount,"#f87171"],
                    ].map(([l,v,c_])=>(
                      <div key={l} style={{ background:"rgba(0,0,0,0.2)",borderRadius:8,padding:"7px 9px",textAlign:"center" }}>
                        <div style={{ fontSize:12,fontWeight:700,color:c_,fontFamily:"'Courier New',monospace" }}>{v}</div>
                        <div style={{ fontSize:9,color:"#3a5a70",marginTop:1 }}>{l}</div>
                      </div>
                    ))}
                  </div>
                  <div style={{ marginTop:8 }}>
                    <ProgressBar value={r.creditScore} color={r.creditColor} height={4}/>
                  </div>
                </div>
              );
            })}
...
// ─── LOGIN SCREEN ─────────────────────────────────────────────────────────────
function LoginScreen({ users, onLogin, isOnline }) {
  ...
  return (
    <div style={{ minHeight:"100vh",background:"#060f1a",display:"flex",alignItems:"center",justifyContent:"center",padding:20,flexDirection:"column",gap:24 }}>
      <div style={{ textAlign:"center" }}>
        ...
        <div style={{ fontSize:11,color:isOnline?"#22c55e":"#f59e0b",marginTop:6,display:"flex",alignItems:"center",justifyContent:"center",gap:5 }}>
          <Icon name="wifi" size={11}/>{isOnline?"Connected to Firebase":"Offline Mode"}
        </div>
      </div>
    ...
    <div>
      ...
      {showAddStaff && (
        <Modal title="Add Loan Officer" onClose={()=>setShowAddStaff(false)}>
          ...
          <div style={{ background:"rgba(96,165,250,0.06)",border:"1px solid rgba(96,165,250,0.15)",borderRadius:10,padding:"10px 12px",marginBottom:14,fontSize:12,color:"#60a5fa" }}>
            This staff will be able to: add clients, record payments, and request loan approvals.
          </div>
      ...
      {editStaff && (
        <Modal title="Edit Staff" onClose={()=>setEditStaff(null)}>
          ...
          <Field label="New PIN (leave blank to keep)"><input type="password" style={inputStyle} placeholder="Enter new PIN..." onChange={e=>{ if(e.target.value) setEditStaff(p=>({...p,pin:e.target.value})); }}/></Field>
...
export default function App() {
  ...
  useEffect(() => {
    setLoading(true);
    let loadedClients = false, loadedUsers = false, loadedPending = false;

    const checkDone = () => {
      if (loadedClients && loadedUsers && loadedPending) setLoading(false);
    };

    const unsubClients = onSnapshot(collection(db, CLIENTS_COL), snap => {
      setClients(snap.docs.map(d => d.data()));
      setIsOnline(true);
      loadedClients = true; checkDone();
    }, () => { loadedClients = true; setIsOnline(false); checkDone(); });

    const unsubUsers = onSnapshot(collection(db, USERS_COL), snap => {
      const u = snap.docs.map(d => d.data());
      setUsers(u.length > 0 ? u : getDefaultUsers());
      loadedUsers = true; checkDone();
    }, () => {
      setUsers(getDefaultUsers());
      loadedUsers = true; checkDone();
    });

    const unsubPending = onSnapshot(collection(db, PENDING_COL), snap => {
      setPendingLoans(snap.docs.map(d => d.data()));
      loadedPending = true; checkDone();
    }, () => { loadedPending = true; checkDone(); });
    ...
    return () => { unsubClients(); unsubUsers(); unsubPending(); };
  }, []);
  ...
  const persist = async (c, u, p) => {
    ...
    try {
      await Promise.all([
        saveClientsToFirebase(c),
        saveUsersToFirebase(u),
        savePendingToFirebase(p),
      ]);
      setIsOnline(true);
  ...
  const handleApproveLoan = (pendingLoan) => {
    ...
    // Also delete from Firebase pending
    deleteFromFirebase(PENDING_COL, pendingLoan.id);
    updateAll(newClients, users, newPending);
  };

  const handleRejectLoan = (pendingId) => {
    deleteFromFirebase(PENDING_COL, pendingId);
  ...
  const handleDeleteClient = (id) => {
    deleteFromFirebase(CLIENTS_COL, id);
  ...
  if (loading) return (
    <div style={{ minHeight:"100vh",background:"#060f1a",display:"flex",alignItems:"center",justifyContent:"center",flexDirection:"column",gap:16 }}>
      ...
      <div style={{ color:"#4a7090",fontSize:14 }}>Connecting to Firebase...</div>
      <div style={{ color:"#2a4050",fontSize:11 }}>creda-finance · Real-time sync</div>
    </div>
  );

  if (!currentUser) return <LoginScreen users={users} onLogin={setCurrentUser} isOnline={isOnline}/>;
  ...
  const Dashboard = () => (
    <div>
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
          <div>
            ...
            <p style={{ margin:"4px 0 0",color:"#3a5a70",fontSize:13 }}>
              {isAdmin?`${clients.length} clients · ${pendingLoans.length} pending`:`${visibleClients.length} my clients`}
            </p>
          </div>
          <div style={{ display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4 }}>
            <RoleBadge role={currentUser.role}/>
            <div style={{ fontSize:10,color:isOnline?"#22c55e":"#f59e0b",display:"flex",alignItems:"center",gap:4 }}>
              <Icon name="wifi" size={10}/>{isOnline?"Live":"Offline"}
            </div>
          </div>
        </div>
      </div>
        ...
        <div onClick={()=>setView("staff")} style={{ background:"rgba(245,158,11,0.08)",border:"1px solid rgba(245,158,11,0.25)",borderRadius:12,padding:"12px 14px",marginBottom:16,cursor:"pointer",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <div style={{ display:"flex",alignItems:"center",gap:8 }}>
            ...
            <div>
              ...
              <div style={{ fontSize:10,color:"#6b5020",marginTop:1 }}>Tap to review</div>
            </div>
          </div>
      ...
      <div style={{ background:"rgba(255,255,255,0.02)",border:"1px solid rgba(100,180,255,0.06)",borderRadius:14,padding:16,marginBottom:20 }}>
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center" }}>
          <div><h3 style={{ margin:0,fontSize:13,fontWeight:700,color:"#93c5fd",textTransform:"uppercase" }}>Monthly Breakdown</h3></div>
    ...
    <div>
      <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20 }}>
        <div>
          ...
          <p style={{ margin:"4px 0 0",color:"#3a5a70",fontSize:12 }}>{filteredClients.length} shown</p>
        </div>
  ...
  const Ledger = () => (
    <div>
      <div style={{ marginBottom:22 }}><h1 style={{ margin:0,fontSize:22,fontWeight:800,color:"#e8f4fd" }}>Ledger Log</h1><p style={{ margin:"4px 0 0",color:"#3a5a70",fontSize:13 }}>{globalTransactions.length} transactions</p></div>
    ...
    return (
      <div>
        ...
        <div style={{ background:"linear-gradient(135deg,rgba(34,197,94,0.08),rgba(59,130,246,0.06))",border:"1px solid rgba(34,197,94,0.2)",borderRadius:16,padding:"16px 18px",marginBottom:16 }}>
            ...
            <>
              ...
              <div>
                <div style={{ display:"flex",justifyContent:"space-between",fontSize:10,color:"#3a5a70",marginBottom:5 }}>
                  ...
                  <span style={{ color:todayData.collected>=todayData.expected?"#4ade80":"#f59e0b" }}>{todayData.expected>0?((todayData.collected/todayData.expected)*100).toFixed(1):0}%</span>
                </div>
                <div style={{ height:6,background:"rgba(255,255,255,0.06)",borderRadius:4,overflow:"hidden" }}>
          ...
          ) : <div style={{ textAlign:"center",color:"#2a4050",fontSize:13 }}>No collections scheduled for today.</div>}
        </div>
                            ...
                            <div key={idx} onClick={()=>{ setSelectedClientId(inst.clientId); setView("detail"); setSelectedDay(null); }} style={{ background:inst.paid?"rgba(34,197,94,0.04)":isShortfall?"rgba(239,68,68,0.06)":"rgba(255,255,255,0.02)",border:`1px solid ${inst.paid?"rgba(34,197,94,0.15)":isShortfall?"rgba(239,68,68,0.2)":"rgba(255,255,255,0.04)"}`,borderRadius:10,padding:"10px 12px",display:"flex",justifyContent:"space-between",alignItems:"center",cursor:"pointer" }}>
                              <div style={{ display:"flex",alignItems:"center",gap:8 }}>
                                ...
                                <div>
                                  ...
                                  {isShortfall&&<div style={{ fontSize:9,color:"#f87171",marginTop:1 }}>Partial: {formatCurrency(inst.paidAmount)}</div>}
                                </div>
                              </div>
                              <div 
